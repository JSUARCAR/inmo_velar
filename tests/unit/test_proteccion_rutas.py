...
async def test_require_login_background(auth_state):
    ...
    with patch(
        ...
        with patch(
            "src.presentacion_reflex.state.auth_state.AuthState.start_navigation_generation"
        ):
            with patch(
                "src.presentacion_reflex.state.auth_state.AuthState.end_navigation_generation"
            ) as mock_end:
                with patch(
                    "src.presentacion_reflex.state.auth_state.AuthState.validate_generation",
                    return_value=True,
                ):
