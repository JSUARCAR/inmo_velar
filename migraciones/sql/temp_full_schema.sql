...
/* VW_ALERTA_MORA_DIARIA(ID_RECAUDO,ID_CONTRATO_A,DIRECCION_PROPIEDAD,ARRENDATARIO,TELEFONO_PRINCIPAL,VALOR_RECAUDO,FECHA_VENCIMIENTO_RECAUDO,FECHA_HOY,DIAS_RETRASO) */;
...
/* VW_ALERTA_VENCIMIENTO_CONTRATOS(ID_CONTRATO_A,DIRECCION_PROPIEDAD,ARRENDATARIO,TELEFONO_PRINCIPAL,FECHA_FIN_CONTRATO_A,DIAS_RESTANTES) */;
...
/* VW_REPORTE_DISPONIBLES(ID_PROPIEDAD,CIUDAD,DIRECCION_PROPIEDAD,ESTRATO,PRECIO,HABITACIONES,BANO,AREA_M2) */;
CREATE VIEW VW_KPI_SIGNOS_VITALES AS
            SELECT
                (SELECT COUNT(*) FROM PROPIEDADES WHERE ESTADO_REGISTRO = 1) AS INVENTARIO_TOTAL,
                (SELECT COUNT(*) FROM PROPIEDADES WHERE DISPONIBILIDAD_PROPIEDAD = 1 AND ESTADO_REGISTRO = 1) AS INVENTARIO_DISPONIBLE,
                (SELECT COALESCE(SUM(CANON_ARRENDAMIENTO), 0) FROM CONTRATOS_ARRENDAMIENTOS WHERE ESTADO_CONTRATO_A = 'Activo') AS RENT_ROLL_ACTIVO,
                ROUND(
                    (CAST((SELECT COUNT(*) FROM PROPIEDADES WHERE DISPONIBILIDAD_PROPIEDAD = 0 AND ESTADO_REGISTRO = 1) AS REAL) / 
                    NULLIF((SELECT COUNT(*) FROM PROPIEDADES WHERE ESTADO_REGISTRO = 1), 0)) * 100, 
                1) AS PORCENTAJE_OCUPACION
/* VW_KPI_SIGNOS_VITALES(INVENTARIO_TOTAL,INVENTARIO_DISPONIBLE,RENT_ROLL_ACTIVO,PORCENTAJE_OCUPACION) */;
CREATE VIEW VW_KPI_YIELD_GAP AS
            WITH Potencial AS (
                SELECT 
                    COALESCE(SUM(
                        CASE 
                            WHEN DISPONIBILIDAD_PROPIEDAD = 1 THEN CANON_ARRENDAMIENTO_ESTIMADO 
                            ELSE 0 
                        END
                    ), 0) as Potencial_Disponible,
                    (SELECT COALESCE(SUM(CANON_ARRENDAMIENTO), 0) FROM CONTRATOS_ARRENDAMIENTOS WHERE ESTADO_CONTRATO_A = 'Activo') as Real_Activo
                FROM PROPIEDADES
                WHERE ESTADO_REGISTRO = 1
            )
            SELECT
                (Potencial_Disponible + Real_Activo) AS POTENCIAL_TOTAL,
                Real_Activo AS INGRESO_REAL,
                ((Potencial_Disponible + Real_Activo) - Real_Activo) AS DINERO_PERDIDO_MENSUAL
            FROM Potencial
/* VW_KPI_YIELD_GAP(POTENCIAL_TOTAL,INGRESO_REAL,DINERO_PERDIDO_MENSUAL) */;
CREATE VIEW VW_KPI_RIESGO_FLUJO_CAJA AS
            SELECT
                (SELECT COUNT(*) FROM RECAUDO_ARRENDAMIENTO WHERE ESTADO_RECAUDO = 'Mora') AS CONTRATOS_EN_MORA,
                (SELECT COALESCE(SUM(VALOR_RECAUDO), 0) FROM RECAUDO_ARRENDAMIENTO WHERE ESTADO_RECAUDO = 'Mora') AS CAPITAL_EN_RIESGO,
                (SELECT COUNT(*) FROM ALERTAS WHERE TIPO_ALERTA = 'Mora Recaudo' AND ESTADO_ALERTA = 'Pendiente') AS ALERTAS_MORA_SIN_GESTION
/* VW_KPI_RIESGO_FLUJO_CAJA(CONTRATOS_EN_MORA,CAPITAL_EN_RIESGO,ALERTAS_MORA_SIN_GESTION) */;
CREATE VIEW VW_AUDITORIA_FORENSE_PAGOS AS
            SELECT 
                L.ID_LIQUIDACION_PROPIETARIO,
                L.VALOR_NETO_PROPIETARIO AS VALOR_AUTORIZADO,
                SUM(P.VALOR_PAGO) AS VALOR_DISPERSADO,
                (SUM(P.VALOR_PAGO) - L.VALOR_NETO_PROPIETARIO) AS DISCREPANCIA
            FROM LIQUIDACIONES_PROPIETARIOS L
            JOIN PAGOS_PROPIETARIOS P ON L.ID_LIQUIDACION_PROPIETARIO = P.ID_LIQUIDACION_PROPIETARIO
            WHERE P.ESTADO_PAGO = 'Pagado'
            GROUP BY L.ID_LIQUIDACION_PROPIETARIO
            HAVING SUM(P.VALOR_PAGO) > L.VALOR_NETO_PROPIETARIO
/* VW_AUDITORIA_FORENSE_PAGOS(ID_LIQUIDACION_PROPIETARIO,VALOR_AUTORIZADO,VALOR_DISPERSADO,DISCREPANCIA) */;
