...
def generate_reports_doble_activos():
    ...
    try:
        # Consulta para obtener solo contratos de mandato y arrendamiento ACTIVOS
        query = """
            SELECT 
                cm.ID_CONTRATO_M,
                ca.ID_CONTRATO_A,
        ...
        # 1. Generar Excel
        excel_path = os.path.join(root_dir, "reporte_auditoria_ambos_activos.xlsx")
        df.to_excel(excel_path, index=False, engine='openpyxl')
        
        # 2. Generar Markdown
        md_path = os.path.join(root_dir, "reporte_auditoria_ambos_activos.md")
        md_table = df.to_markdown(index=False)
        with open(md_path, "w", encoding="utf-8") as f:
            f.write("# Reporte de Auditoría: Mandato Activo y Arrendamiento Activo\n\n")
            f.write(md_table)
...
if __name__ == "__main__":
    generate_reports_doble_activos()
