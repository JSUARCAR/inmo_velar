...
def procesar_mandatos(cursor, mode_commit: bool) -> List[Dict[str, Any]]:
    ...
    if mode_commit and discrepancias:
        ...
        """
        updates = [(d["nueva_fecha_pago"], d["nuevo_grupo"], d["id"]) for d in discrepancias]
        cursor.executemany(query_update, updates)
        logger.info(f"Se actualizaron atomicamente {len(discrepancias)} contratos de mandato.")
        
    return discrepancias

def main():
    parser = argparse.ArgumentParser(description="Script para recalibrar los grupos de pago de los Contratos de Mandato.")
    parser.add_argument("--commit", action="store_true", help="Aplica los cambios en la BD transaccionalmente (por defecto es dry-run).")
    ...
    try:
        if args.commit:
            with db_manager.transaccion() as conexion:
                ...
                diffs = procesar_mandatos(cursor, mode_commit=True)
            logger.info("=== Ejecucion Completada. Transaccion confirmada (COMMIT). ===")
        else:
            conexion = db_manager.obtener_conexion()
            cursor = db_manager.get_dict_cursor(conexion)
            diffs = procesar_mandatos(cursor, mode_commit=False)
            logger.info("=== Ejecucion en modo Dry-Run finalizada. No se altero la base de datos. ===")
