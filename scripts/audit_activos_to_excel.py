...
def generate_excel_report_activos():
    ...
    try:
        # Consulta para obtener solo contratos ACTIVOS (Mandato y Arrendamiento)
        query = """
            SELECT 
                cm.ID_CONTRATO_M,
                ca.ID_CONTRATO_A,
        ...
        output_path = os.path.join(root_dir, "reporte_coincidencia_activos.xlsx")
        
        # Exportar a Excel
        df.to_excel(output_path, index=False, engine='openpyxl')
            
        print(f"Reporte Excel de Activos generado exitosamente en: {output_path}")
