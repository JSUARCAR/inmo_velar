        ...
        with self.db_manager.obtener_conexion() as conn:
            ...
            if row:
                if hasattr(row, 'keys') or isinstance(row, dict):
                    bonificacion.id_bonificacion_asesor = row.get('ID_BONIFICACION_ASESOR') or row.get('id_bonificacion_asesor')
                    bonificacion.fecha_registro = row.get('FECHA_REGISTRO') or row.get('fecha_registro')
                else:
