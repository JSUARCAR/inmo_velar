...
class ContratoMandatoElite(BaseDocumentTemplate):
    ...
    def validate_data(self, data: Dict[str, Any]) -> bool:
        ...
        self._require_fields(
            data["mandante"], "nombre", "documento", "telefono", "email"
        )
        ...
        if not DataValidator.validate_money_amount(
            data["condiciones"].get("comision", 0)
        ):
    ...
    def generate(self, data: Dict[str, Any]) -> Path:
        ...
        identificador = data.get("inmueble", {}).get(
            "direccion", str(data.get("contrato_id", "0"))
        )
        filename = self._generate_filename("contrato_mandato", identificador)
        ...
        self.add_paragraph(
            f"<b>FECHA DE SUSCRIPCIÓN DEL CONTRATO:</b> {self._format_fecha_es(fecha_sistema)}",
            alignment="LEFT",
            leftIndent=0,
        )
        self.add_paragraph(
            "<b>CIUDAD DEL CONTRATO:</b> ARMENIA, QUINDÍO", alignment="LEFT"
        )
        # self.add_paragraph("ARMENIA, QUINDÍO", alignment='LEFT')
        self.add_spacer(0.4)
        ...
        self.add_spacer(0.2)
        self.add_paragraph(
            f"Para constancia de lo anterior se firma el día <b>{self._format_fecha_es(fecha_sistema)}</b>, en dos ejemplares de un mismo tenor literal ordenado por la Ley."
        )
        self.add_spacer(0.5)
    ...
    def _add_resumen_partes(self, data: Dict[str, Any]):
        ...
        mandatario = data.get(
            "inmobiliaria", self.ARRENDADOR_INFO
        ...
        def p_kw(txt):
            return Paragraph(f"<b>{txt}</b>", style_label)
        ...
        def p_val(txt):
            ...
            return [
                Paragraph(line.strip(), style_val) for line in lines if line.strip()
            ]
        ...
        row_inm = [[p_kw("DIRECCIÓN DEL INMUEBLE:"), p_val(direccion_full)]]

        # Mandatario (Inmobiliaria)
        # Usamos datos quemados si no vienen, o los del objeto
        nombre_inmo = html.escape(
            str(mandatario.get("nombre", "INMOBILIARIA VELAR S.A.S."))
        )
        nit_inmo = html.escape(str(mandatario.get("nit", "901703515-7")))
        rep_legal = html.escape(
            str(mandatario.get("representante", "CRISTIAN FERNANDO JAMIOY FONSECA"))
        )
        ...
        row_arr = [[p_kw("MANDATARIO:"), p_val(contenido_mandatario)]]
        ...
        data_table.append(
            [p_kw("CANON MANDATO:"), p_val(f"{canon_fmt} (COP - PESOS COLOMBIANOS)")]
        )
        data_table.append([p_kw("FECHA DE INICIO DEL CONTRATO:"), p_val(fecha_inicio)])
        data_table.append(
            [p_kw("FECHA DE TERMINACIÓN DEL CONTRATO:"), p_val(fecha_fin)]
        )
    ...
    def _add_clausulas_reales(self, data: Dict[str, Any]):
        ...
        fecha_pago_num = "5"
        comision_porcentaje_texto = "DIEZ"
        ...
        try:
            ...
            duracion_meses = data["condiciones"].get(
                "duracion_meses", 12
            )  # Fallback al valor numérico directo si falla cálculo

            if (
                f_inicio_str
                and f_fin_str
                and f_inicio_str != "N/A"
                and f_fin_str != "N/A"
            ):
                try:
            ...
            # 3. Comisión Porcentaje
            comision_val = data["condiciones"].get("comision", 10)
            comision_porcentaje = str(comision_val)
            comision_porcentaje_texto = num2words(comision_val, lang="es").upper()
        ...
        mapeo = {
            ...
            "[MATRICULA INMOBILIARIA]": str(
                data["inmueble"].get("matricula_inmobiliaria", "N/A")
            ),
            "[FECHA ACTUAL DEL SISTEMA]": self._format_fecha_es(
                data.get("fecha", "N/A")
            ),
            "[NOMBRE PROPIETARIO]": str(mandante.get("nombre", "N/A")).upper(),
            "[NOMBRE CONSIGNATARIO]": str(
                mandante.get("consignatario") or mandante.get("nombre", "N/A")
            ).upper(),
            ...
            "[TIPO DE CUENTA PROPIETARIO]": str(
                mandante.get("tipo_cuenta", "___TIPO___")
            ),
            "[NUMERO DE CUENTA PROPIETARIO]": str(
                mandante.get("numero_cuenta", "___NUMERO___")
            ),
            ...
            "[DIFERENCIA DE MESES FECHA FIN - FECHA INICIO]": str(
                data["condiciones"].get("duracion_meses", 12)
            ),
            ...
            "[PORCENTAJE COMISION TEXTO]": comision_porcentaje_texto,
        }
        ...
        for clausula in self.CLAUSULAS_TEXTO:
            ...
            for k, v in mapeo.items():
                if k in texto:
                    ...
                    )  # Simplificado para evitar tags anidadas
            ...
            for part in parts:
                if part.strip():
                    self.add_paragraph(
                        part.strip(), style_name="Body", alignment="justify"
                    )
    ...
    def _add_firmas_tres_columnas(self, data: Dict[str, Any]):
        ...
        style_firma = ParagraphStyle(
            "Firma", fontName="Helvetica", fontSize=8, leading=10, alignment=1
        )  # Center
        ...
        # Mandante
        bloque_2 = firma_bloque("MANDANTE", mandante["nombre"], mandante["documento"])

        # Tabla de 2 columnas
        tabla_firmas = Table([[bloque_1, bloque_2]], colWidths=[250, 250])
