...
class ContratoArrendamientoElite(BaseDocumentTemplate):
    ...
    def generate(self, data: Dict[str, Any]) -> Path:
        ...
        self.add_paragraph(
            "<b>CIUDAD DEL CONTRATO:</b><br/>ARMENIA, QUINDÍO", align="CENTER"
        )
        self.add_spacer(0.4)
    ...
    def _add_resumen_partes(self, data: Dict[str, Any]):
        ...
        row_prop = [[p_kw("DIRECCIÓN DEL INMUEBLE:"), p_val(direccion_completa)]]
        ...
        row_user = [
            [
                p_kw("ARRENDATARIO:"),
                p_val(f"{arrendatario['nombre']}<br/>C.C. {arrendatario['documento']}"),
            ]
        ]
