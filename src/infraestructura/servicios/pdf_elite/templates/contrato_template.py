...
class ContratoArrendamientoElite(BaseDocumentTemplate):
    ...
    def __init__(self, output_dir: Path = None):
        ...
        self.document_title = (
            "CONTRATO DE ARRENDAMIENTO DE INMUEBLE DESTINADO A VIVIENDA URBANA"
        )
    ...
    def validate_data(self, data: Dict[str, Any]) -> bool:
        ...
        self._require_fields(
            data["arrendatario"], "nombre", "documento", "telefono", "email"
        )
        self._require_fields(
    ...
    def generate(self, data: Dict[str, Any]) -> Path:
        ...
        filename = self._generate_filename("contrato_arrendamiento", identificador)
        ...
        self.add_paragraph(
            f"<b>FECHA DE SUSCRIPCIÓN DEL CONTRATO:</b> {self._format_date_spanish(fecha_sistema)}",
            alignment="LEFT",
            leftIndent=0,
        )
        self.add_paragraph(
            "<b>CIUDAD DEL CONTRATO:</b><br/>ARMENIA, QUINDÍO",
            alignment="LEFT",
            leftIndent=0,
        )
        # self.add_spacer(0.4)
    ...
    def _add_resumen_partes(self, data: Dict[str, Any]):
        ...
        def p_val(txt):
            return Paragraph(str(txt), style_val)
        ...
        row_arr = [
            [
                p_kw("ARRENDADOR:"),
                p_val(
                    f"{arrendador['nombre']}<br/>NIT: {arrendador['nit']}<br/>REP: {arrendador['representante']}<br/>C.C: {arrendador['documento_rep']}"
                ),
            ]
        ]
        ...
        if (
            inmueble.get("municipio")
            and inmueble.get("departamento")
            and inmueble.get("municipio") != "N/A"
        ):
            direccion_completa += f"<br/>{inmueble['municipio'].upper()}, {inmueble['departamento'].upper()}"

        row_inm = [[p_kw("DIRECCIÓN DEL INMUEBLE:"), p_val(direccion_completa)]]

        # Canon y Fechas
        row_eco = [
            [p_kw("CANON DE ARRENDAMIENTO:"), p_val(canon_fmt)],
            [
                p_kw("FECHA DE INICIO:"),
                p_val(self._format_date_spanish(data["fecha_inicio"])),
            ],
        ]

        # Arrendatario
        datos_arrendatario = (
            f"{arrendatario['nombre']}<br/>C.C. {arrendatario['documento']}"
        )
        if (
            arrendatario.get("codigo_seguro")
            and arrendatario.get("codigo_seguro") != "N/A"
        ):
            datos_arrendatario += (
                f"<br/><b>CÓDIGO APROBACIÓN SEGURO:</b> {arrendatario['codigo_seguro']}"
            )

        row_user = [[p_kw("ARRENDATARIO:"), p_val(datos_arrendatario)]]

        # Codeudor y Fecha Fin
        row_cod = [
            [
                p_kw("CODEUDOR:"),
                p_val(f"{codeudor['nombre']}<br/>C.C. {codeudor['documento']}"),
            ]
        ]
        row_fin = [
            [
                p_kw("FECHA DE TERMINACIÓN:"),
                p_val(self._format_date_spanish(data["fecha_fin"])),
            ]
        ]
        ...
        tiene_codeudor = (
            codeudor
            and codeudor.get("documento")
            and codeudor.get("documento") != "N/A"
        )
        if tiene_codeudor:
        ...
        data_table.append(
            [p_kw("CANON ARRENDAMIENTO:"), p_val(f"{canon_fmt}<br/>{canon_texto}")]
        )
        # 6. INICIO
        data_table.append(
            [
                p_kw("FECHA DE INICIO DEL CONTRATO:"),
                p_val(self._format_date_spanish(data["fecha_inicio"])),
            ]
        )
        # 7. FIN
        data_table.append(
            [
                p_kw("FECHA DE TERMINACIÓN DEL CONTRATO:"),
                p_val(self._format_date_spanish(data["fecha_fin"])),
            ]
        )
        # 8. DURACIÓN
        data_table.append(
            [
                p_kw("DURACIÓN DEL CONTRATO:"),
                p_val(f"{cond.get('duracion_meses', 12)} Meses"),
            ]
        )
    ...
    def _add_clausulas_reales(self, data: Dict[str, Any]):
        ...
        mapeo = {
            ...
            "[DIFERENCIA DE MESES FECHA FIN - FECHA INICIO]": str(
                data["condiciones"]["duracion_meses"]
            ),  # Simplificado, asumimos viene calculado o data raw
            "[VALOR CANON ARRENDAMIENTO EN TEXTO]": num2words(
                data["condiciones"]["canon"], lang="es"
            ).upper()
            + " PESOS M/CTE",
            # --- NUEVOS PLACEHOLDERS ---
    ...
    def _add_firmas_tres_columnas(self, data: Dict[str, Any]):
        ...
        if tiene_codeudor:
            ...
            tabla_firmas = Table(
                [[bloque_1, bloque_2, bloque_3]], colWidths=[170, 170, 170]
            )
        else:
            tabla_firmas = Table([[bloque_1, bloque_2]], colWidths=[255, 255])
