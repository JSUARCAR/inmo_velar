...
class ContratoArrendamientoElite(BaseDocumentTemplate):
    ...
    def _add_condiciones_economicas(self, data: Dict[str, Any]) -> None:
        ...
        self.add_spacer(0.2)
        self.add_paragraph(
            f"<b>Duración:</b> {cond['duracion_meses']} meses",
            style_name='Body'
        )
        if not es_mandato:
            self.add_paragraph(
            ...
            self.add_paragraph(
                f"<b>Incremento Anual:</b> IPC certificado por el DANE",
                style_name='Body'
            )
        self.add_spacer(0.3)
    ...
    def _add_firmas(self, data: Dict[str, Any]) -> None:
        ...
        self.add_paragraph(
            ...
            "en la ciudad de Bogotá D.C., a los {fecha}.".format(
                fecha=data['fecha']
            ),
            style_name='Body'
        )
