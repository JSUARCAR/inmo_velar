...
class ReciboRecaudoElite(BaseDocumentTemplate):
    ...
    def _layout_info_blocks(self, data: Dict[str, Any]):
        ...
        style_label_head.textColor = Colors.to_reportlab(
            Colors.GRAY_DARKEST
        )  # Color dorado/acento
        style_label_head.fontSize = 11
        style_label_head.alignment = (
            TA_LEFT  # <--- Asegurar alineación izquierda explícita
        )
        ...
        propiedad_content = [
            ...
            [
                Paragraph(data.get("propiedad", "N/A").upper(), style_val_bold)
            ],  # Dirección en negrita
            [
                Paragraph(
                    f"Municipio: {data.get('municipio', 'ARMENIA')} | Dept: {data.get('departamento', 'QUINDÍO')}",
                    style_val,
                )
            ],
    ...
    def _layout_table(self, data: Dict[str, Any]):
        ...
        if not conceptos:
            ...
            periodo_fmt = self._format_periodo_es(data.get("periodo")).capitalize()
            rows.append(
                ["Canon de Arrendamiento", periodo_fmt, f"${data['valor_total']:,.0f}"]
            )
        else:
            for c in conceptos:
        ...
        style = [
            ...
            (
                ...
                (-1, 0),
                accent_col,
            ),  # Encabezado Dorado según referencia LOGOPET
    ...
    def _layout_footer_totals(self, data: Dict[str, Any]):
        ...
        left_content = [
            ...
            Paragraph(
                f"<b>Referencia:</b> {data.get('referencia_pago', 'N/A')}", style_normal
            ),
            Paragraph(
                f"<b>Banco/Cuenta:</b> {data.get('banco', 'N/A')} - {data.get('tipo_cuenta', '')}",
                style_normal,
            ),
            Spacer(1, 0.2 * cm),
        ]
        ...
        # Grid Contenedor
        main_footer = Table(
            [[left_content, t_totals]], colWidths=[4.5 * inch, 3.0 * inch]
        )
        main_footer.setStyle(
            TableStyle(
                [
                    ("VALIGN", (0, 0), (-1, -1), "TOP"),
    ...
    def _layout_contact_footer(self, data: Dict[str, Any]):
        ...
        text = [
            ...
            Paragraph(
                "Este recibo constituye prueba de pago. Consérvelo para sus registros.",
                self.styles["Small"],
            ),
        ]
