...
class EstadoCuentaElite(BaseDocumentTemplate):
    ...
    def validate_data(self, data: Dict[str, Any]) -> bool:
        ...
        self._require_fields(
            data, "estado_id", "propietario", "inmueble", "periodo", "movimientos", "resumen"
        )
        return True
    ...
    def _add_header_estado(self, data: Dict[str, Any]) -> None:
        ...
        self.add_document_info_header(
            doc_number=f"No. {data['estado_id']:06d}", doc_date=f"Período: {data['periodo']}"
        )
    ...
    def _add_resumen_financiero(self, data: Dict[str, Any]) -> None:
        ...
        if "cuenta_bancaria" in resumen and valor_neto > 0:
            self.add_spacer(0.2)
            self.add_paragraph(f"Valor a Transferir: ${valor_neto:,.2f}", style_name="Body")
