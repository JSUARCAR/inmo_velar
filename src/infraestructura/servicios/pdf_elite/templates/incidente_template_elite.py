...
class IncidenteTemplateElite(BaseDocumentTemplate):
    ...
    def generate(self, data: Dict[str, Any]) -> Path:
        ...
        filename = self._generate_filename(
            f"incidente_{data.get('estado','').lower()}", data["id"]
        )
    ...
    def _layout_info_general(self, data):
        ...
        sol_block = [
            ...
            Paragraph(
                f"Responsable Pago: <b>{data.get('responsable_pago', 'Por definir')}</b>",
                s_val,
            ),
        ]
    ...
    def _layout_cotizaciones(self, data):
        ...
        if not cots:
            self.story.append(
                Paragraph("No hay cotizaciones registradas.", self.styles["Body"])
            )
            return
            ...
            TableStyle(
                [
                    (
                        ...
                        (0, 0),
                        (-1, 0),
                        Colors.to_reportlab(Colors.GRAY_DARKEST),
                    ),
    ...
    def _layout_orden_trabajo(self, data, titulo="DATOS DE EJECUCIÓN"):
        ...
        info = [
            [
                Paragraph(
                    f"<b>PROVEEDOR ASIGNADO:</b><br/>{prov_name}", self.styles["Body"]
                ),
                Paragraph(
                    f"<b>FECHA PROGRAMADA/EJECUCIÓN:</b><br/>{fecha_arreglo}",
                    self.styles["Body"],
                ),
                Paragraph(
                    f"<b>VALOR APROBADO:</b><br/>{costo}", self.styles["Heading2"]
                ),
            ]
        ]
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (-1, -1),
                        1,
                        Colors.to_reportlab(Colors.GRAY_MEDIUM),
                    ),
    ...
    def _layout_cierre_final(self, data):
        ...
        obs = data.get(
            "observaciones_final",
            "La reparación ha sido completada satisfactoriamente.",
        )
        if not obs:
            obs = "Sin observaciones adicionales."

        self.story.append(
            Paragraph(f"<b>Observaciones Finales:</b> {obs}", self.styles["Body"])
        )
        self.story.append(Spacer(1, 0.2 * cm))
        self.story.append(
            Paragraph(
                "<i>Se certifica que el trabajo ha sido recibido a entera satisfacción y cumple con los estándares de calidad requeridos.</i>",
                self.styles["Small"],
            )
        )
    ...
    def _layout_firmas(self):
        ...
        s_centered = ParagraphStyle(
            "Centered", parent=self.styles["Body"], alignment=TA_CENTER
        )
