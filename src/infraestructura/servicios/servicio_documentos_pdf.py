...
class PDFGenerator(FPDF):
    ...
    def __init__(self, title: str, logo_data: str = None):
        super().__init__()
    ...
    def header(self):
        ...
        if self.logo_data:
            try:
                ...
                try:
                    # Centrar Logo
                    # Ancho página A4 = 210mm. Márgenes default = 10mm.
            ...
            self.cell(
                0,
                5,
                "NIT: 900.123.456-7 | Dirección: Calle 100 # 15-20, Bogotá",
                align="C",
                new_x="LMARGIN",
                new_y="NEXT",
            )
            self.cell(
                0,
                5,
                "Teléfono: (601) 555-5555 | Email: contacto@inmovelar.com",
                align="C",
                new_x="LMARGIN",
                new_y="NEXT",
            )
...
class ServicioDocumentosPDF:
    ...
    def generar_comprobante_recaudo(self, datos: Dict[str, Any]) -> str:
        ...
        pdf.cell(40, 7, "Comprobante No:", border=0)
        ...
        # Fila 2
        pdf.set_font("helvetica", "B", 10)
        pdf.cell(40, 7, "Contrato:", border=0)
        pdf.set_font("helvetica", "", 10)
        pdf.cell(
            150,
            7,
            f"{datos['id_contrato_a']} - {datos.get('direccion_propiedad', 'N/A')}",
            border=0,
            new_x="LMARGIN",
            new_y="NEXT",
        )
        ...
        # Fila 3
        pdf.set_font("helvetica", "B", 10)
        pdf.cell(40, 7, "Método Pago:", border=0)
        pdf.set_font("helvetica", "", 10)
        pdf.cell(60, 7, f"{datos['metodo_pago']}", border=0)

        pdf.set_font("helvetica", "B", 10)
        pdf.cell(30, 7, "Referencia:", border=0)
        pdf.set_font("helvetica", "", 10)
        pdf.cell(
            60,
            7,
            f"{datos.get('referencia_bancaria', 'N/A')}",
            border=0,
            new_x="LMARGIN",
            new_y="NEXT",
        )
        ...
        pdf.cell(
            0,
            5,
            f"Observaciones: {datos.get('observaciones', 'Ninguna')}",
            new_x="LMARGIN",
            new_y="NEXT",
        )
        pdf.ln(20)
    ...
    def generar_estado_cuenta(self, datos: Dict[str, Any]) -> str:
        ...
        pdf.cell(
            190,
            7,
            f"Documento: {datos['documento']} | Matrícula: {datos['matricula']}",
            border=0,
            new_x="LMARGIN",
            new_y="NEXT",
        )
        pdf.cell(
            190,
            7,
            f"Banco: {datos.get('banco', 'N/A')} | Cuenta: {datos.get('tipo_cuenta', '')} - {datos.get('numero_cuenta', '')}",
            border=0,
            new_x="LMARGIN",
            new_y="NEXT",
        )
        ...
        pdf.set_font("helvetica", "B", 12)
        pdf.cell(140, 10, "NETO A PAGAR AL PROPIETARIO", border=1, fill=True, align="C")
        pdf.cell(
            50,
            10,
            f"${datos['neto_pagar']:,}",
        ...
        if datos["estado"] == "Pagada":
            ...
            pdf.cell(
                60,
                6,
                f"Referencia: {datos['referencia_pago']}",
                border=0,
                new_x="LMARGIN",
                new_y="NEXT",
            )
    ...
    def generar_cuenta_cobro_asesor(self, datos: Dict[str, Any]) -> str:
        ...
        pdf.cell(
            100,
            7,
            f"Documento: {datos['documento_asesor']}",
            border=0,
            new_x="LMARGIN",
            new_y="NEXT",
        )
        ...
        pdf.cell(
            95,
            8,
            f"Liquidación #: {datos['id_liquidacion']}",
            border=1,
            new_x="LMARGIN",
            new_y="NEXT",
        )
