...
class Settings(BaseSettings):
    ...
    database_path: str = Field(
        default="migraciones/DB_Inmo_Velar.db", description="Ruta al archivo SQLite"
    )
    ...
    db_name: str = Field(
        default="db_inmo_velar", description="Nombre de la base de datos PostgreSQL"
    )

    # === Aplicación ===
    app_name: str = Field(
        default="Sistema de Gestión Inmobiliaria", description="Nombre de la aplicación"
    )
    ...
    comision_default_porcentaje: float = Field(
        default=10.0, description="Porcentaje de comisión por defecto"
    )

    # === Notificaciones (Email Office 365) ===
    smtp_server: str = Field(default="smtp.office365.com", description="Servidor SMTP")
    ...
    wa_autosend_delay: float = Field(
        default=3.5, description="Tiempo de espera para envío automático WhatsApp"
    )
