...
class RepositorioPersonaSQLite:
    ...
    def _row_to_entity(self, row) -> Persona:
        ...
        return Persona(
            ...
            telefono_principal=(row_dict.get('telefono_principal') or row_dict.get('TELEFONO_PRINCIPAL')),
            correo_electronico=(row_dict.get('correo_electronico') or row_dict.get('CORREO_ELECTRONICO')),
            direccion_principal=(row_dict.get('direccion_principal') or row_dict.get('DIRECCION_PRINCIPAL')),
    ...
    def obtener_por_id(self, id_persona: int) -> Optional[Persona]:
        ...
        cursor.execute(
            f"SELECT * FROM PERSONAS WHERE ID_PERSONA = {placeholder}",
            (id_persona,)
        )
    ...
    def obtener_por_documento(self, numero_documento: str) -> Optional[Persona]:
        ...
        cursor.execute(
            f"SELECT * FROM PERSONAS WHERE NUMERO_DOCUMENTO = {placeholder}",
            (numero_documento,)
        )
    ...
    def crear(self, persona: Persona, usuario_sistema: str) -> Persona:
        ...
        cursor.execute(
            ...
            (
                ...
                bool(persona.estado_registro) if persona.estado_registro is not None else True,  # PostgreSQL boolean
