...
class RepositorioPermisos:
    ...
    def obtener_permiso(self, id_permiso: int) -> Optional[Permiso]:
        ...
        cursor.execute(
            f"SELECT * FROM PERMISOS WHERE ID_PERMISO = {placeholder}",
            (id_permiso,)
        )
    ...
    def crear_permiso(self, permiso: Permiso) -> Permiso:
        ...
        try:
            cursor.execute(
                ...
                """,
                (permiso.modulo, permiso.ruta, permiso.accion, permiso.descripcion, permiso.categoria)
            )
    ...
    def asignar_permiso_a_rol(self, rol: str, id_permiso: int, usuario: str) -> RolPermiso:
        ...
        try:
            cursor.execute(
                ...
                """,
                (rol, id_permiso, usuario, datetime.now().isoformat(), usuario, datetime.now().isoformat())
            )
    ...
    def limpiar_permisos_rol(self, rol: str) -> bool:
        ...
        try:
            cursor.execute(
                f"DELETE FROM public.ROL_PERMISOS WHERE ROL = {placeholder}",
                (rol,)
            )
