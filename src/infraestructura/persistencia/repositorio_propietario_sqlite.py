...
class RepositorioPropietarioSQLite:
    ...
    def _row_to_entity(self, row: sqlite3.Row) -> Propietario:
        ...
        return Propietario(
            ...
            observaciones_propietario=(row_dict.get('observaciones_propietario') or row_dict.get('OBSERVACIONES_PROPIETARIO')),
            estado_propietario=(row_dict.get('estado_propietario') or row_dict.get('ESTADO_PROPIETARIO')),
            fecha_ingreso_propietario=(row_dict.get('fecha_ingreso_propietario') or row_dict.get('FECHA_INGRESO_PROPIETARIO')),
    ...
    def obtener_por_id(self, id_propietario: int) -> Optional[Propietario]:
        ...
        cursor.execute(
            f"SELECT * FROM PROPIETARIOS WHERE ID_PROPIETARIO = {placeholder}",
            (id_propietario,)
        )
    ...
    def obtener_por_persona(self, id_persona: int) -> Optional[Propietario]:
        ...
        cursor.execute(
            f"SELECT * FROM PROPIETARIOS WHERE ID_PERSONA = {placeholder}",
            (id_persona,)
        )
    ...
    def listar_activos(self) -> List[Propietario]:
        ...
        cursor.execute(
            "SELECT * FROM PROPIETARIOS WHERE ESTADO_PROPIETARIO = 1"
        )
    ...
    def crear(self, propietario: Propietario, usuario_sistema: str) -> Propietario:
        ...
        with self.db.obtener_conexion() as conn:
            ...
            conn.commit()
            propietario.id_propietario = self.db.get_last_insert_id(cursor, 'PROPIETARIOS', 'ID_PROPIETARIO')
