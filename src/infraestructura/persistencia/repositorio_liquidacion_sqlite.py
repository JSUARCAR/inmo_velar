...
class RepositorioLiquidacionSQLite:
    ...
    def _row_to_entity(self, row) -> Liquidacion:
        ...
        return Liquidacion(
            ...
            comision_porcentaje=(row_dict.get('comision_porcentaje') or row_dict.get('COMISION_PORCENTAJE')),
            ...
            gastos_administracion=(row_dict.get('gastos_administracion') or row_dict.get('GASTOS_ADMINISTRACION')),
            gastos_servicios=(row_dict.get('gastos_servicios') or row_dict.get('GASTOS_SERVICIOS')),
            gastos_reparaciones=(row_dict.get('gastos_reparaciones') or row_dict.get('GASTOS_REPARACIONES')),
            ...
            estado_liquidacion=(row_dict.get('estado_liquidacion') or row_dict.get('ESTADO_LIQUIDACION')),
            ...
            motivo_cancelacion=(row_dict.get('motivo_cancelacion') or row_dict.get('MOTIVO_CANCELACION')),
    ...
    def crear(self, liquidacion: Liquidacion, usuario_sistema: str) -> Liquidacion:
        ...
        liquidacion.id_liquidacion = self.db.get_last_insert_id(cursor, 'LIQUIDACIONES', 'ID_LIQUIDACION')
        conn.commit()
    ...
    def obtener_por_id(self, id_liquidacion: int) -> Optional[Liquidacion]:
        ...
        cursor.execute(f"SELECT * FROM LIQUIDACIONES WHERE ID_LIQUIDACION = {placeholder}", (id_liquidacion,))
        row = cursor.fetchone()
    ...
    def listar_todas(self):
        ...
        for row in cursor.fetchall():
            ...
            results.append({
                ...
                'id_contrato_m': (row_dict.get('id_contrato_m') or row_dict.get('ID_CONTRATO_M')),
                'direccion': (row_dict.get('direccion_propiedad') or row_dict.get('DIRECCION_PROPIEDAD')),
                'matricula': (row_dict.get('matricula_inmobiliaria') or row_dict.get('MATRICULA_INMOBILIARIA')),
                'propietario': (row_dict.get('nombre_propietario') or row_dict.get('NOMBRE_PROPIETARIO')),
                ...
                'impuesto': (row_dict.get('impuesto_4x1000') or row_dict.get('IMPUESTO_4X1000')),
                'gastos_admin': (row_dict.get('gastos_administracion') or row_dict.get('GASTOS_ADMINISTRACION')),
                'gastos_servicios': (row_dict.get('gastos_servicios') or row_dict.get('GASTOS_SERVICIOS')),
                'gastos_reparaciones': (row_dict.get('gastos_reparaciones') or row_dict.get('GASTOS_REPARACIONES')),
                'otros_egresos': (row_dict.get('otros_egresos') or row_dict.get('OTROS_EGRESOS')),
                'total_egresos': (row_dict.get('total_egresos') or row_dict.get('TOTAL_EGRESOS')),
                'neto': (row_dict.get('neto_a_pagar') or row_dict.get('NETO_A_PAGAR')),
                'estado': (row_dict.get('estado_liquidacion') or row_dict.get('ESTADO_LIQUIDACION')),
                'fecha_pago': (row_dict.get('fecha_pago') or row_dict.get('FECHA_PAGO')),
                'observaciones': (row_dict.get('observaciones') or row_dict.get('OBSERVACIONES')),
                'created_at': (row_dict.get('created_at') or row_dict.get('CREATED_AT'))
            })
    ...
    def actualizar(self, liquidacion: Liquidacion, usuario_sistema: str) -> None:
        ...
        """, (
            liquidacion.fecha_generacion,
            liquidacion.canon_bruto, liquidacion.otros_ingresos, liquidacion.total_ingresos,
            liquidacion.comision_porcentaje, liquidacion.comision_monto, liquidacion.iva_comision, liquidacion.impuesto_4x1000,
            liquidacion.gastos_administracion, liquidacion.gastos_servicios, liquidacion.gastos_reparaciones, liquidacion.otros_egresos,
            liquidacion.total_egresos, liquidacion.neto_a_pagar,
            liquidacion.observaciones,
            datetime.now().isoformat(),
            usuario_sistema,
    ...
    def marcar_como_pagada(self, id_liquidacion: int, fecha_pago: str, metodo_pago: str, 
                          referencia_pago: str, usuario_sistema: str) -> None:
    ...
    def cancelar_masivamente(self, ids_liquidaciones: List[int], motivo: str, usuario_sistema: str) -> dict:
        """
    ...
    def cancelar_por_propietario_y_periodo(self, id_propietario: int, periodo: str, motivo: str, usuario_sistema: str) -> int:
        """
    ...
    def reversar_por_propietario_y_periodo(self, id_propietario: int, periodo: str, usuario_sistema: str) -> int:
        """
    ...
    def listar_por_propietario_y_periodo(self, id_propietario: int, periodo: str) -> List[Liquidacion]:
