...
class RepositorioContratoArrendamientoSQLite:
    ...
    def crear(self, contrato: ContratoArrendamiento, usuario: str) -> ContratoArrendamiento:
        ...
        """, (
            contrato.id_propiedad, contrato.id_arrendatario, contrato.id_codeudor,
            contrato.fecha_inicio_contrato_a, contrato.fecha_fin_contrato_a, contrato.duracion_contrato_a,
            contrato.canon_arrendamiento, contrato.deposito,
            contrato.estado_contrato_a, contrato.alerta_vencimiento_contrato_a, contrato.alerta_ipc,
            contrato.fecha_renovacion_contrato_a, contrato.fecha_incremento_ipc,
            usuario, usuario
        ))
        
        conn.commit()
        contrato.id_contrato_a = self.db.get_last_insert_id(cursor, 'CONTRATOS_ARRENDAMIENTOS', 'ID_CONTRATO_A')
    ...
    def obtener_por_id(self, id_contrato: int) -> Optional[ContratoArrendamiento]:
        ...
        placeholder = self.db.get_placeholder()
        cursor.execute(f"SELECT * FROM CONTRATOS_ARRENDAMIENTOS WHERE ID_CONTRATO_A = {placeholder}", (id_contrato,))
        row = cursor.fetchone()
    ...
    def listar_todos(self) -> List[ContratoArrendamiento]:
        ...
        cursor.execute(f"SELECT * FROM CONTRATOS_ARRENDAMIENTOS ORDER BY ID_CONTRATO_A DESC")
    ...
    def _row_to_entity(self, row) -> ContratoArrendamiento:
        ...
        return ContratoArrendamiento(
            ...
            fecha_inicio_contrato_a=(row_dict.get('fecha_inicio_contrato_a') or row_dict.get('FECHA_INICIO_CONTRATO_A')),
            fecha_fin_contrato_a=(row_dict.get('fecha_fin_contrato_a') or row_dict.get('FECHA_FIN_CONTRATO_A')),
            duracion_contrato_a=(row_dict.get('duracion_contrato_a') or row_dict.get('DURACION_CONTRATO_A')),
            canon_arrendamiento=(row_dict.get('canon_arrendamiento') or row_dict.get('CANON_ARRENDAMIENTO')),
            deposito=(row_dict.get('deposito') or row_dict.get('DEPOSITO')),
            estado_contrato_a=(row_dict.get('estado_contrato_a') or row_dict.get('ESTADO_CONTRATO_A')),
            motivo_cancelacion=(row_dict.get('motivo_cancelacion') or row_dict.get('MOTIVO_CANCELACION')),
            alerta_vencimiento_contrato_a=(row_dict.get('alerta_vencimiento_contrato_a') or row_dict.get('ALERTA_VENCIMIENTO_CONTRATO_A')),
            alerta_ipc=(row_dict.get('alerta_ipc') or row_dict.get('ALERTA_IPC')),
            fecha_renovacion_contrato_a=(row_dict.get('fecha_renovacion_contrato_a') or row_dict.get('FECHA_RENOVACION_CONTRATO_A')),
            fecha_incremento_ipc=(row_dict.get('fecha_incremento_ipc') or row_dict.get('FECHA_INCREMENTO_IPC')),
            ...
            # Asumamos que sí.
            fecha_ultimo_incremento_ipc=(row_dict.get('fecha_ultimo_incremento_ipc') or row_dict.get('FECHA_ULTIMO_INCREMENTO_IPC')),
