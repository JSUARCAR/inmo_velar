...
class RepositorioIPCPostgres:
    """Repositorio Postgres para la entidad IPC."""

    def __init__(self, db_manager: DatabaseManager):
        self.db = db_manager

    def _row_to_entity(self, row: Postgres3.Row) -> IPC:
    ...
    def obtener_por_id(self, id_ipc: int) -> Optional[IPC]:
        ...
        cursor.execute(f"SELECT * FROM IPC WHERE ID_IPC = {placeholder}", (id_ipc,))
    ...
    def obtener_por_anio(self, anio: int) -> Optional[IPC]:
        ...
        cursor.execute(f"SELECT * FROM IPC WHERE ANIO = {placeholder}", (anio,))
    ...
    def actualizar(self, ipc: IPC, usuario_sistema: str) -> bool:
        ...
        with self.db.obtener_conexion() as conn:
            ...
            cursor.execute(
                ...
                """,
                (ipc.valor_ipc, ipc.fecha_publicacion, ipc.id_ipc),
            )
