...
class RepositorioContratoMandatoSQLite:
    ...
    def crear(self, contrato: ContratoMandato, usuario: str) -> ContratoMandato:
        ...
        """, (
            contrato.id_propiedad, contrato.id_propietario, contrato.id_asesor,
            contrato.fecha_inicio_contrato_m, contrato.fecha_fin_contrato_m, contrato.duracion_contrato_m,
            contrato.canon_mandato, contrato.comision_porcentaje_contrato_m, contrato.iva_contrato_m,
            contrato.estado_contrato_m, contrato.alerta_vencimiento_contrato_m, contrato.fecha_renovacion_contrato_m,
            usuario, usuario
        ))
        
        conn.commit()
        contrato.id_contrato_m = self.db.get_last_insert_id(cursor, 'CONTRATOS_MANDATOS', 'ID_CONTRATO_M')
        return contrato
    ...
    def obtener_por_id(self, id_contrato: int) -> Optional[ContratoMandato]:
        ...
        placeholder = self.db.get_placeholder()
        cursor.execute(f"SELECT * FROM CONTRATOS_MANDATOS WHERE ID_CONTRATO_M = {placeholder}", (id_contrato,))
        row = cursor.fetchone()
    ...
    def listar_por_propietario_activos(self, id_propietario: int) -> List[ContratoMandato]:
        ...
        """, (id_propietario,)) # ERROR EN ORIGINAL: WHERE ID_PROPIEDAD = id_propietario, pero el método se llama listar_por_propietario_activos y el argumento es id_propietario.
    ...
    def _row_to_entity(self, row) -> ContratoMandato:
        ...
        return ContratoMandato(
            ...
            fecha_inicio_contrato_m=(row_dict.get('fecha_inicio_contrato_m') or row_dict.get('FECHA_INICIO_CONTRATO_M')),
            fecha_fin_contrato_m=(row_dict.get('fecha_fin_contrato_m') or row_dict.get('FECHA_FIN_CONTRATO_M')),
            duracion_contrato_m=(row_dict.get('duracion_contrato_m') or row_dict.get('DURACION_CONTRATO_M')),
            canon_mandato=(row_dict.get('canon_mandato') or row_dict.get('CANON_MANDATO')),
            comision_porcentaje_contrato_m=(row_dict.get('comision_porcentaje_contrato_m') or row_dict.get('COMISION_PORCENTAJE_CONTRATO_M')),
            iva_contrato_m=(row_dict.get('iva_contrato_m') or row_dict.get('IVA_CONTRATO_M')),
            estado_contrato_m=(row_dict.get('estado_contrato_m') or row_dict.get('ESTADO_CONTRATO_M')),
            ...
            # We preserve it.
            alerta_vencimiento_contrato_m=(row_dict.get('alerta_vencimineto_contrato_m') or row_dict.get('ALERTA_VENCIMINETO_CONTRATO_M')),
            fecha_renovacion_contrato_m=(row_dict.get('fecha_renovacion_contrato_m') or row_dict.get('FECHA_RENOVACION_CONTRATO_M')),
