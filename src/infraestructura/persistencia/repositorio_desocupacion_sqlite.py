...
class RepositorioDesocupacionSQLite:
    ...
    def crear(self, desocupacion: Desocupacion) -> Desocupacion:
        ...
        with self.db_manager.obtener_conexion() as conn:
            ...
            id_desocupacion = self.db_manager.get_last_insert_id(cursor, 'DESOCUPACIONES', 'ID_DESOCUPACION')
    ...
    def listar_todas(self, estado: Optional[str] = None) -> List[Desocupacion]:
        ...
        with self.db_manager.obtener_conexion() as conn:
            ...
            if estado:
                cursor.execute(f"{query} WHERE d.ESTADO = {placeholder} ORDER BY d.FECHA_PROGRAMADA ASC", (estado,))
            else:
    ...
    def actualizar_estado(self, id_desocupacion: int, nuevo_estado: str, usuario: str, fecha_real: Optional[str] = None):
        ...
        with self.db_manager.obtener_conexion() as conn:
            ...
            query = f"""
                ...
                WHERE ID_DESOCUPACION = {placeholder}
            """
            cursor.execute(query, (nuevo_estado, fecha_real, datetime.now().isoformat(), usuario, id_desocupacion))
            conn.commit()
