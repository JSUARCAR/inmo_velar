...
def crear_liquidacion_form_view(
    ...
    if es_edicion:
        try:
            ...
            if not contrato:
                raise ValueError(f"No se encontró el contrato de mandato ID {liquidacion_base.id_contrato_m}")
            
            id_propietario = contrato.id_propietario
            liquidacion_periodo = liquidacion_base.periodo
            ...
            liquidaciones = servicio_financiero.repo_liquidacion.listar_por_propietario_y_periodo(
                id_propietario, 
                liquidacion_base.periodo
            )
            
            if not liquidaciones:
                raise ValueError(f"No se encontraron liquidaciones para propietario {id_propietario} en periodo {liquidacion_base.periodo}")
            ...
            observaciones_consolidadas = " | ".join(observaciones_list) if observaciones_list else ""
    ...
    def calcular_totales(e=None):
        ...
        try:
            ...
            total_egr = comision_monto + iva + impuesto + gastos_admin + gastos_serv + gastos_rep + otros_egr
    ...
    try:
        ...
        contratos_options = [
            ft.dropdown.Option(key=str(m["id"]), text=m["texto"])
            for m in mandatos
        ]
    except Exception as e:
        ...
        content=ft.Column([
            ...
            ft.Row([
                ...
                ft.TextField(
                    ...
                    label="Período *",
                    hint_text="YYYY-MM",
                    value=liquidacion_propietario_info['periodo'] if liquidacion_propietario_info else datetime.now().strftime("%Y-%m"),
                    width=150,
                    disabled=es_edicion
                ),
        ...
        content=ft.Column([
            ...
            ft.Row([
                ft.TextField(
                    ...
                    label="Canon Bruto *",
                    hint_text="0",
                    value=str(liquidacion_propietario_info['suma_canon_bruto']) if liquidacion_propietario_info else "",
                    prefix_text="$",
                    width=200,
                ...
                ft.TextField(
                    ...
                    hint_text="0",
                    prefix_text="$",
                    value=str(liquidacion_propietario_info['suma_otros_ingresos']) if liquidacion_propietario_info else "0",
                    width=200,
                    keyboard_type=ft.KeyboardType.NUMBER,
                    on_change=calcular_totales
                ),
    ...
    seccion_egresos = ft.Container(
        content=ft.Column([
            ft.Text("Egresos y Deducciones", size=18, weight=ft.FontWeight.BOLD, color="#f44336"),
            ft.Divider(height=1, color="#e0e0e0"),
            ...
            ft.Row([
                ft.TextField(
                    ...
                    label="Porcentaje",
                    hint_text="1000 = 10%",
                    value=str(liquidacion_propietario_info['comision_porcentaje']) if liquidacion_propietario_info else "",
                    width=150,
                    keyboard_type=ft.KeyboardType.NUMBER,
                    on_change=calcular_totales
                ),
            ...
            ft.Row([
                ft.TextField(
                    ...
                    hint_text="0",
                    prefix_text="$",
                    value=str(liquidacion_propietario_info['suma_gastos_admin']) if liquidacion_propietario_info else "0",
                    width=180,
                    keyboard_type=ft.KeyboardType.NUMBER,
                    on_change=calcular_totales
                ),
                ft.TextField(
                    ...
                    hint_text="0",
                    prefix_text="$",
                    value=str(liquidacion_propietario_info['suma_gastos_servicios']) if liquidacion_propietario_info else "0",
                    width=180,
                    keyboard_type=ft.KeyboardType.NUMBER,
                    on_change=calcular_totales
                ),
                ft.TextField(
                    ...
                    hint_text="0",
                    prefix_text="$",
                    value=str(liquidacion_propietario_info['suma_gastos_reparaciones']) if liquidacion_propietario_info else "0",
                    width=180,
                    keyboard_type=ft.KeyboardType.NUMBER,
                    on_change=calcular_totales
                ),
                ft.TextField(
                    ...
                    hint_text="0",
                    prefix_text="$",
                    value=str(liquidacion_propietario_info['suma_otros_egresos']) if liquidacion_propietario_info else "0",
                    width=180,
                    keyboard_type=ft.KeyboardType.NUMBER,
                    on_change=calcular_totales
                ),
    ...
    seccion_obs = ft.Container(
        content=ft.Column([
            ...
            ft.TextField(
                ...
                value=liquidacion_propietario_info['observaciones'] if liquidacion_propietario_info else "",
                multiline=True,
                min_lines=3,
                max_lines=5,
            ),
