...
def crear_poliza_form_view(page: ft.Page, on_guardar, on_cancelar):
    ...
    dd_contrato = ft.Dropdown(
        ...
        options=[
            ft.dropdown.Option(
                key=str(c["ID_CONTRATO_A"]), text=f"{c['DIRECCION']} - {c['INQUILINO']}"
            )
            for c in contratos
        ],
        expand=True,
    ...
    txt_numero_poliza = ft.TextField(
        label="Número de Póliza (Opcional)", hint_text="Ej: POL-123456"
    )
    ...
    txt_fecha_inicio = ft.TextField(
        label="Vigencia Desde *", read_only=True, width=160, value=today.strftime("%Y-%m-%d")
    )
    txt_fecha_fin = ft.TextField(
        label="Vigencia Hasta *", read_only=True, width=160, value=next_year.strftime("%Y-%m-%d")
    )
        ...
        content=ft.Column(
            [
                                ...
                                ft.Row(
                                    [
                                        ...
                                        ft.IconButton(
                                            ...
                                            on_click=lambda e: page.open(date_inicio),
                                        ),
                                    ]
                                ),
                            ]
                            ...
                            [
                                ...
                                ft.Row(
                                    [
                                        ...
                                        ft.IconButton(
                                            ft.Icons.CALENDAR_MONTH,
                                            on_click=lambda e: page.open(date_fin),
                                        ),
                                    ]
                                ),
                            ]
                ...
                ft.Row(
                    [
                        ft.ElevatedButton(
                            "Cancelar", on_click=lambda e: on_cancelar(), bgcolor=colors.SECONDARY
                        ),
                        ft.ElevatedButton(
                            "Guardar Póliza",
                            on_click=guardar,
