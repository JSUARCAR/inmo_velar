...
def crear_seguro_form_view(
    ...
    if es_edicion:
        ...
        if not seguro_actual:
            page.open(ft.SnackBar(
                ft.Row([ft.Icon(ft.Icons.ERROR_OUTLINE, color=colors.TEXT_ON_ERROR), ft.Text("Seguro no encontrado", color=colors.TEXT_ON_ERROR)]),
    ...
    date_picker_inicio = ft.DatePicker(
        ...
        value=datetime.datetime.strptime(seguro_actual.fecha_inicio_seguro, "%Y-%m-%d") if seguro_actual and seguro_actual.fecha_inicio_seguro else datetime.datetime.now()
    )
    ...
    txt_fecha_inicio = ft.TextField(
        ...
        width=180,
        read_only=True,
        value=seguro_actual.fecha_inicio_seguro if seguro_actual and seguro_actual.fecha_inicio_seguro else datetime.datetime.now().date().isoformat()
    )
    
    btn_fecha_inicio = ft.IconButton(
        icon=ft.Icons.CALENDAR_MONTH,
        on_click=abrir_picker_fecha,
    ...
    formulario = ft.Container(
        content=ft.Column(
            [
                ...
                ft.Container(
                    content=ft.Column([
                        ft.Text(f"Inicio > Seguros > {'Editar' if es_edicion else 'Nuevo'}", style=styles.breadcrumb_text()),
                        ft.Text(
                            titulo,
                            size=24,
                ...
                ft.Container(
                    content=ft.Column(
                        [
                            ...
                            txt_nombre,
                            ft.Row(
                                [txt_porcentaje, txt_fecha_inicio, btn_fecha_inicio],
                                spacing=20
                            ),
                ...
                ft.Container(
                    content=ft.Column(
                        [
                            ...
                            ft.Container(
                                content=ft.Column([
                                    ft.Text("Porcentaje en base 10000:", weight=ft.FontWeight.BOLD, size=13),
                                    ft.Text("• 100 = 1.0%", size=12, color=colors.TEXT_SECONDARY),
                                    ft.Text("• 200 = 2.0%", size=12, color=colors.TEXT_SECONDARY),
                                    ft.Text("• 300 = 3.0%", size=12, color=colors.TEXT_SECONDARY),
                                    ft.Text("• 400 = 4.0%", size=12, color=colors.TEXT_SECONDARY),
                                ], spacing=5),
