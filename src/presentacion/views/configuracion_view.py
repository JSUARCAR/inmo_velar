...
class ConfiguracionView(ft.Container):
    def __init__(
        ...
        tab_parametros = ft.Tab(
            ...
            content=ft.Container(
                ...
                content=ft.Column([
                    ft.Row([
                        ft.Text("Parámetros del Sistema", size=24, weight=ft.FontWeight.BOLD),
                    ]),
                    ft.Divider(),
                    ft.Text(
            ...
            content=ft.Container(
                ...
                content=ft.Column([
                    ft.Row([
                        ...
                        ft.Container(expand=True),
                        ft.IconButton(icon=ft.Icons.REFRESH, on_click=lambda e: self.cargar_auditoria())
                    ]),
                    ft.Divider(),
                    ft.Text(
        ...
        self.content = ft.Column([
            ft.Container(
                content=ft.Row([
                    ...
                    ft.Text("Configuración del Sistema", size=28, weight=ft.FontWeight.BOLD),
                ]),
    ...
    def on_tab_change(self, e):
        ...
        if idx == 0: self.cargar_usuarios()
        elif idx == 1: self.cargar_ipc()
        elif idx == 2: self.cargar_parametros()
        elif idx == 3: self.cargar_auditoria()

    def cargar_usuarios(self):
        self.loading_usuarios.visible = True
        if self.usuarios_ref.current: self.usuarios_ref.current.visible = False
        self.update()
        
        inactivos = self.mostrar_inactivos.current.value if self.mostrar_inactivos.current else False
    ...
    def _update_usuarios_ui(self, usuarios):
        ...
        for u in usuarios:
            ...
            rows.append(ft.DataRow(cells=[
                ...
                ft.DataCell(ft.Container(content=ft.Text(u.rol, color="white", size=12), bgcolor=ft.Colors.BLUE if u.rol == "Administrador" else ft.Colors.TEAL, border_radius=5, padding=ft.padding.symmetric(horizontal=8, vertical=4))),
                ft.DataCell(ft.Container(content=ft.Text(estado_texto, color="white", size=12), bgcolor=estado_color, border_radius=5, padding=ft.padding.symmetric(horizontal=8, vertical=4))),
                ft.DataCell(ft.Text(u.ultimo_acceso[:16] if u.ultimo_acceso else "-")),
                ft.DataCell(ft.Row([
                    ft.IconButton(ft.Icons.EDIT, icon_color=ft.Colors.BLUE, tooltip="Editar", on_click=lambda e, uid=u.id_usuario: self.on_editar_usuario(uid)),
                    ft.IconButton(ft.Icons.LOCK_RESET, icon_color=ft.Colors.ORANGE, tooltip="Resetear", on_click=lambda e, uid=u.id_usuario: self.mostrar_dialog_reset(uid)),
                    ft.IconButton(ft.Icons.BLOCK if u.es_activo() else ft.Icons.CHECK_CIRCLE, icon_color=ft.Colors.RED if u.es_activo() else ft.Colors.GREEN, tooltip="Desactivar" if u.es_activo() else "Reactivar", on_click=lambda e, uid=u.id_usuario, act=u.es_activo(): self.toggle_estado_usuario(uid, act)),
                ]))
            ]))
    ...
    def cargar_ipc(self):
        ...
        if self.ipc_ref.current: self.ipc_ref.current.visible = False
        self.update()
    ...
    def _update_ipc_ui(self, lista):
        ...
        for ipc in lista:
            rows.append(ft.DataRow(cells=[
                ...
                ft.DataCell(ft.Text(ipc.fecha_publicacion[:10] if ipc.fecha_publicacion else "-")),
                ft.DataCell(ft.IconButton(ft.Icons.EDIT, icon_color=ft.Colors.BLUE, tooltip="Editar", on_click=lambda e, i=ipc: self.mostrar_dialog_editar_ipc(i))),
            ]))
    ...
    def cargar_parametros(self):
        ...
        if self.parametros_container_ref.current: self.parametros_container_ref.current.visible = False
        self.update()
    ...
    def _update_params_ui(self, data):
        ...
        for cat, params in data.items():
            ...
            for p in params:
                ...
                campos_cat.append(ft.Container(content=ft.Row([txt, ft.Text(f"({p.tipo_dato})", size=12, color="grey")]), margin=ft.margin.only(bottom=10)))

            btn = ft.ElevatedButton("Guardar Configuración", icon=ft.Icons.SAVE, bgcolor=ft.Colors.BLUE, color=ft.Colors.WHITE,
            ...
            panels.append(ft.ExpansionPanel(
                header=ft.ListTile(leading=self.obtener_icono_categoria(cat), title=ft.Text(cat, weight="bold"), subtitle=ft.Text(f"{len(params)} parámetros")),
                content=ft.Container(content=ft.Column(campos_cat, spacing=5), padding=20)
            ))
            
        if self.parametros_container_ref.current:
            self.parametros_container_ref.current.controls = [ft.ExpansionPanelList(elevation=1, expand_icon_color=ft.Colors.BLUE, controls=panels)]
    ...
    def cargar_auditoria(self):
        ...
        if self.auditoria_ref.current: self.auditoria_ref.current.visible = False
        self.update()
    ...
    def _get_accion_color(self, accion):
        if accion == 'INSERT': return ft.Colors.GREEN
        if accion == 'UPDATE': return ft.Colors.ORANGE
        if accion == 'DELETE': return ft.Colors.RED
    ...
    def obtener_icono_categoria(self, cat):
        iconos = {"COMISIONES": ft.Icons.PERCENT, "IMPUESTOS": ft.Icons.ACCOUNT_BALANCE, "VALIDACIONES": ft.Icons.RULE, 
                  "ALERTAS": ft.Icons.NOTIFICATIONS, "OPERACIONES": ft.Icons.SETTINGS, "NOTIFICACIONES": ft.Icons.MESSAGE, "SISTEMA": ft.Icons.COMPUTER}
        return ft.Icon(iconos.get(cat, ft.Icons.SETTINGS))

    def toggle_estado_usuario(self, uid, activo):
        try:
            if activo: self.servicio.desactivar_usuario(uid, self.usuario_actual.nombre_usuario)
            else: self.servicio.reactivar_usuario(uid, self.usuario_actual.nombre_usuario)
            self.cargar_usuarios()
    ...
    def mostrar_dialog_reset(self, uid):
        ...
        def confirmar(e):
            try:
                self.servicio.resetear_contrasena(uid, txt.value, self.usuario_actual.nombre_usuario)
                self.page_ref.close(dlg)
                self.page_ref.snack_bar = ft.SnackBar(ft.Text("Contraseña reseteada"), bgcolor="green"); self.page_ref.snack_bar.open=True; self.page_ref.update()
            except Exception as ex: self._show_error(str(ex))
        
        dlg = ft.AlertDialog(title=ft.Text("Resetear"), content=txt, 
                             actions=[ft.TextButton("Cancelar", on_click=lambda e: self.page_ref.close(dlg)), ft.ElevatedButton("Confirmar", on_click=confirmar)])
    ...
    def mostrar_dialog_nuevo_ipc(self):
        ...
        def confirmar(e):
            try:
                self.servicio.agregar_ipc(int(anio.value), int(float(val.value)*100), self.usuario_actual.nombre_usuario)
                self.page_ref.close(dlg); self.cargar_ipc()
            except Exception as ex: self._show_error(str(ex))

        dlg = ft.AlertDialog(title=ft.Text("Nuevo IPC"), content=ft.Column([anio, val], tight=True),
                             actions=[ft.TextButton("Cancelar", on_click=lambda e: self.page_ref.close(dlg)), ft.ElevatedButton("Guardar", on_click=confirmar)])
        self.page_ref.open(dlg)
    ...
    def mostrar_dialog_editar_ipc(self, ipc):
        ...
        def confirmar(e):
            try:
                self.servicio.actualizar_ipc(ipc.id_ipc, int(float(val.value)*100), usuario_sistema=self.usuario_actual.nombre_usuario)
                self.page_ref.close(dlg); self.cargar_ipc()
            except Exception as ex: self._show_error(str(ex))
            
        dlg = ft.AlertDialog(title=ft.Text(f"Editar IPC {ipc.anio}"), content=val,
                             actions=[ft.TextButton("Cancelar", on_click=lambda e: self.page_ref.close(dlg)), ft.ElevatedButton("Guardar", on_click=confirmar)])
        self.page_ref.open(dlg)
    ...
    def guardar_categoria(self, cat, params):
        try:
            ...
            if valores:
                cnt = self.servicio.actualizar_parametros_por_categoria(cat, valores, self.usuario_actual.nombre_usuario)
                self.page_ref.snack_bar = ft.SnackBar(ft.Text(f"{cnt} parámetros actualizados"), bgcolor="green")
            else:
                 self.page_ref.snack_bar = ft.SnackBar(ft.Text("No hay cambios"), bgcolor="orange")
            self.page_ref.snack_bar.open=True; self.page_ref.update()
        except Exception as e: self._show_error(str(e))

def build_configuracion_view(page, servicio_configuracion, usuario_actual, on_nuevo_usuario, on_editar_usuario):
    return ConfiguracionView(page, servicio_configuracion, usuario_actual, on_nuevo_usuario, on_editar_usuario)
