...
def crear_recibo_publico_form_view(
        ...
        try:
            ...
            if not dropdown_propiedad.current.value:
                page.snack_bar = ft.SnackBar(
                    content=ft.Text("Debe seleccionar una propiedad"), bgcolor="red"
                )
            ...
            if not txt_periodo.current.value:
                page.snack_bar = ft.SnackBar(
                    content=ft.Text("El período es obligatorio"), bgcolor="red"
                )
                page.snack_bar.open = True
                page.update()
                return
            ...
            if not dropdown_tipo_servicio.current.value:
                page.snack_bar = ft.SnackBar(
                    content=ft.Text("Debe seleccionar el tipo de servicio"), bgcolor="red"
                )
                page.snack_bar.open = True
                page.update()
                return
            ...
            if not txt_valor.current.value:
                page.snack_bar = ft.SnackBar(
                    content=ft.Text("El valor es obligatorio"), bgcolor="red"
                )
                page.snack_bar.open = True
                page.update()
                return
            ...
            datos = {
                ...
                "fecha_vencimiento": (
                    txt_fecha_vencimiento.current.value
                    if txt_fecha_vencimiento.current.value
                    else None
                ),
            ...
            page.snack_bar = ft.SnackBar(
                content=ft.Text(f"Error de validación: {str(ve)}"), bgcolor="red"
            )
            page.snack_bar.open = True
            page.update()
    ...
    try:
        ...
        for p in propiedades:
            ...
            opciones_propiedades.append(
                ft.dropdown.Option(key=str(id_prop), text=f"{direccion} - {municipio}")
            )
    except Exception:
    ...
    if modo == "edit":
        ...
        if not recibo_actual:
            page.snack_bar = ft.SnackBar(content=ft.Text("No se encontró el recibo"), bgcolor="red")
            page.snack_bar.open = True
            page.update()
            on_cancelar()
            return ft.Container()
    ...
    breadcrumbs = ft.Row(
        [
            ...
            ft.Text(" > ", size=14, color="#666"),
            ft.Text(
                "Editar" if modo == "edit" else "Nuevo",
                size=14,
                            ...
                            ft.ResponsiveRow(
                                [
                                    ft.TextField(
                                        ...
                                        read_only=True,
                                        value=(
                                            recibo_actual.fecha_pago
                                            if recibo_actual and recibo_actual.fecha_pago
                                            else ""
                                        ),
                                        suffix=ft.IconButton(
                                            icon=ft.Icons.CALENDAR_MONTH,
                                    ...
                                    ft.TextField(
                                        ...
                                        value=(
                                            recibo_actual.comprobante
                                            if recibo_actual and recibo_actual.comprobante
                                            else ""
                                        ),
                                    ),
                                ]
                ...
                ft.Container(
                    content=ft.Column(
                        [
                            ...
                            (
                                DocumentManager(
                                    entidad_tipo="RECIBO_PUBLICO",
                                    entidad_id=str(id_recibo),
                                    page=page,
                                )
                                if modo == "edit"
                                else ft.Text(
                                    "Guarde el recibo para adjuntar documentos",
                                    color="grey",
                                    italic=True,
                                )
                            ),
