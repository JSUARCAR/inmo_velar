...
class RecibosPublicosListView(ft.Container):
    def __init__(
            ...
            content=ft.Column([
                ...
                ft.ResponsiveRow([
                    ...
                    ft.Dropdown(
                        ...
                        hint_text="Todos",
                        options=[
                            ft.dropdown.Option("Agua"), ft.dropdown.Option("Luz"), ft.dropdown.Option("Gas"),
                            ft.dropdown.Option("Internet"), ft.dropdown.Option("Teléfono"), ft.dropdown.Option("Aseo"),
                    ...
                    ft.Dropdown(
                        ...
                        options=[
                            ...
                            ft.dropdown.Option("Pagado"), ft.dropdown.Option("Vencido"),
                        ],
                        on_change=lambda e: self.cargar_datos(mantener_opciones=True)
                    ),
                ]),
        ...
        resumen_section = ft.Container(
            content=ft.ResponsiveRow([
                self._crear_card_resumen("Total Pendiente", ft.Icons.PENDING_ACTIONS, "#ff9800", self.res_pend),
                self._crear_card_resumen("Total Vencido", ft.Icons.WARNING, "#f44336", self.res_venc),
                self._crear_card_resumen("Total Pagado", ft.Icons.CHECK_CIRCLE, "#4caf50", self.res_pagd),
            ]),
        ...
        self.content = ft.Column([
            # Breadcrumbs & Header
                ft.Row([
                ft.Text("Inicio", size=14, color="#666"), ft.Text(" > ", size=14, color="#666"),
                ft.Text("Recibos Públicos", size=14, weight=ft.FontWeight.BOLD, color="#1976d2"),
            ]),
            ft.Divider(height=20, color="transparent"),
            ft.Row([
                ft.Text("Gestión de Recibos Públicos", size=28, weight=ft.FontWeight.BOLD),
                ft.Container(expand=True),
                ft.ElevatedButton("+ Nuevo Recibo", icon=ft.Icons.ADD, on_click=lambda e: self.on_nuevo_recibo(), bgcolor="#1976d2", color="white", height=45)
            ...
            ft.Container(
                ref=self.results_container_ref,
                expand=True
            )
        ...
        return ft.Container(
            ...
            content=ft.Container(
                content=ft.Column([
                    ...
                    ft.Text(ref=ref, value="$0", size=24, weight=ft.FontWeight.BOLD, color=color)
            ...
            if not mantener_opciones:
                try:
                    ...
                    for p in props:
                        ...
                        opciones.append(ft.dropdown.Option(key=str(p.id_propiedad), text=f"{p.direccion_propiedad} - {muni}"))
    ...
    def _calcular_resumen_propiedad(self, recibos):
        ...
        for recibo in recibos:
            ...
            if tipo not in servicios_resumen:
                servicios_resumen[tipo] = {'total': 0, 'pendiente': 0, 'vencido': 0, 'pagado': 0, 'count': 0}
            
            data = servicios_resumen[tipo]
            data['total'] += recibo.valor_recibo
            data['count'] += 1
            if recibo.esta_pagado: data['pagado'] += recibo.valor_recibo
            elif recibo.esta_vencido: data['vencido'] += recibo.valor_recibo
            else: data['pendiente'] += recibo.valor_recibo
        return servicios_resumen

    def _schedule_ui_update(self, resumen=None, resumen_props=None, update_opciones=False, error=None):
        if error:
            if self.results_container_ref.current:
        ...
        if resumen_props:
            ...
            for s, d in resumen_props.items():
                filas_res.append(ft.DataRow(cells=[
                    ft.DataCell(ft.Text(s)), ft.DataCell(ft.Text(str(d['count']))),
                    ft.DataCell(ft.Text(f"${d['total']:,}")), ft.DataCell(ft.Text(f"${d['pendiente']:,}")),
                    ft.DataCell(ft.Text(f"${d['vencido']:,}")), ft.DataCell(ft.Text(f"${d['pagado']:,}")),
                ]))
            
            tbl_res = ft.DataTable(
                columns=[ft.DataColumn(ft.Text("Servicio")), ft.DataColumn(ft.Text("#")), ft.DataColumn(ft.Text("Total")),
                         ft.DataColumn(ft.Text("Pend")), ft.DataColumn(ft.Text("Venc")), ft.DataColumn(ft.Text("Pag"))],
                rows=filas_res,
            ...
            content_results.append(
                ft.Container(
                    content=ft.Column([
                        ft.Text("Resumen de Propiedad", weight="bold", size=16),
                        tbl_res
                    ]), 
                    bgcolor="white", padding=20, border_radius=8, border=ft.border.all(1, "#e0e0e0")
                )
    ...
    def _crear_tabla_recibos(self):
        if not self.data_recibos:
             return ft.Container(content=ft.Text("No hay recibos", size=16, color="#999"), padding=50, alignment=ft.alignment.center)

        filas = []
        for rec in self.data_recibos:
            color = {'Pagado': '#4caf50', 'Pendiente': '#ff9800', 'Vencido': '#f44336'}.get(rec.estado, 'grey')
            acciones = [
                ft.IconButton(ft.Icons.VISIBILITY, icon_color="#1976d2", tooltip="Ver", 
                              on_click=lambda e, id=rec.id_recibo_publico: self.on_ver_detalle(id) if self.on_ver_detalle else None)
            ]
            
            if not rec.esta_pagado:
                 acciones.append(ft.IconButton(ft.Icons.EDIT, icon_color="#2196f3", tooltip="Editar",
                    on_click=lambda e, id=rec.id_recibo_publico: self.on_editar_recibo(id)))
                 acciones.append(ft.IconButton(ft.Icons.PAYMENT, icon_color="#4caf50", tooltip="Pagar",
                    on_click=lambda e, id=rec.id_recibo_publico: self.on_marcar_pagado(id)))
                 acciones.append(ft.IconButton(ft.Icons.DELETE, icon_color="#f44336", tooltip="Eliminar",
            ...
            filas.append(ft.DataRow(cells=[
                ...
                ft.DataCell(ft.Text(self.cache_propiedades.get(rec.id_propiedad, 'N/A')[:20] + "...")),
                ...
                ft.DataCell(ft.Container(content=ft.Text(rec.estado, color="white", size=11, weight="bold"), bgcolor=color, padding=5, border_radius=4)),
        ...
        return ft.Container(
            content=ft.DataTable(
                columns=[
                    ft.DataColumn(ft.Text("Servicio")), ft.DataColumn(ft.Text("Propiedad")),
                    ft.DataColumn(ft.Text("Período")), ft.DataColumn(ft.Text("Valor")),
                    ft.DataColumn(ft.Text("Vence")), ft.DataColumn(ft.Text("Estado")),
...
def crear_recibos_publicos_list_view(page, servicio_recibos_publicos, servicio_propiedades, servicio_notificaciones, on_nuevo_recibo, on_editar_recibo, on_marcar_pagado, on_eliminar_recibo, on_ver_detalle=None):
    return RecibosPublicosListView(page, servicio_recibos_publicos, servicio_propiedades, servicio_notificaciones, on_nuevo_recibo, on_editar_recibo, on_marcar_pagado, on_eliminar_recibo, on_ver_detalle)
