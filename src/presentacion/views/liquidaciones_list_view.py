...
class LiquidacionesListView(ft.Container):
    def __init__(
        ...
        on_ver_detalle: Callable[
            [int, str], None
        ],  # Cambiado: ahora recibe (id_propietario, periodo)
        on_aprobar: Callable[[int, str], None],  # Cambiado: ahora recibe (id_propietario, periodo)
        on_marcar_pagada: Callable[
            [int, str], None
        ],  # Cambiado: ahora recibe (id_propietario, periodo)
            ...
            [
                ...
                ft.Row(
                    [
                        ...
                        ft.Text(
                            "Liquidaciones", size=14, weight=ft.FontWeight.BOLD, color="#1976d2"
                        ),
                    ]
                ),
    ...
    def _schedule_ui_update(self, error=None):
        ...
        for liq in self.liquidaciones_data:
            ...
            if liq["estado"] == "En Proceso":
                ...
                acciones.append(
                    ft.IconButton(
                        ...
                        on_click=lambda e, id_prop=liq["id_propietario"], per=liq[
                            "periodo"
                        ]: self.on_editar(id_prop, per),
                    )
                )
                # Aprobar
                acciones.append(
                ...
                acciones.append(
                    ft.IconButton(
                        ...
                        on_click=lambda e, id_prop=liq["id_propietario"], per=liq[
                            "periodo"
                        ]: self.on_marcar_pagada(id_prop, per),
                    )
                )
            ...
            acciones.append(
                ft.IconButton(
                    ...
                    on_click=lambda e, id_prop=liq["id_propietario"], per=liq[
                        "periodo"
                    ]: self.on_ver_detalle(id_prop, per),
                )
            )
                ...
                ft.DataRow(
                    cells=[
                        ...
                        ft.DataCell(
                            ft.Container(
                                content=ft.Text(
                                    str(liq["cantidad_contratos"]), weight=ft.FontWeight.BOLD
                                ),
                                bgcolor="#e3f2fd",
                        ...
                        ft.DataCell(
                            ft.Container(
                                content=ft.Text(
                                    liq["estado"], color="white", size=12, weight=ft.FontWeight.BOLD
                                ),
        ...
        self.tabla_container.current.controls = [
            ft.Container(content=tabla, border_radius=8, bgcolor="white", padding=10)
        ]
        self.tabla_container.current.update()
    ...
    def _abrir_dialogo_masivas(self):
        ...
        def generar_masivas(e):
            ...
            filtro_asesor = (
                dropdown_asesor.current.value if dropdown_asesor.current.value != "Todos" else None
            )
            ...
            def _generar_thread():
                try:
                    ...
                    liquidaciones = self.servicio_financiero.generar_liquidaciones_masivas(
                        periodo=periodo, filtro_asesor=filtro_asesor_int, usuario_sistema=usuario
                    )
                    ...
                    async def _actualizar_ui_exito():
                        ...
                        self.page_ref.snack_bar = ft.SnackBar(
                            content=ft.Text(
                                f"✅ {len(liquidaciones)} liquidación(es) generada(s) exitosamente"
                            ),
                            bgcolor="#4caf50",
                        )
                ...
                content=ft.Column(
                    [
                        ...
                        ft.Container(
                            content=ft.Column(
                                [
                                    ft.Text(ref=txt_progress, value="", size=12, color="#666"),
...
def crear_liquidaciones_list_view(
    page,
    servicio_financiero,
