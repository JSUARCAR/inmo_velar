...
def crear_contrato_arrendamiento_form_view(
    ...
    dd_arrendatario = ft.Dropdown(
        ...
        options=[
            ft.dropdown.Option(
                key=str(p.datos_roles["Arrendatario"].id_arrendatario), 
    ...
    dd_codeudor = ft.Dropdown(
        ...
        options=[
            ft.dropdown.Option(
                key=str(p.datos_roles["Codeudor"].id_codeudor), 
                text=p.nombre_completo
            )
            for p in codeudores
        ],
    ...
    # Económicos
    txt_canon = ft.TextField(label="Canon Arriendo *", prefix_text="$ ", width=200, keyboard_type=ft.KeyboardType.NUMBER, read_only=True)
    txt_deposito = ft.TextField(label="Depósito", prefix_text="$ ", width=200, keyboard_type=ft.KeyboardType.NUMBER)
    ...
    if es_edicion:
        ...
        if arriendo:
            ...
            if not val_exists:
                dd_propiedad.options.append(ft.dropdown.Option(
    ...
    def handle_guardar(e):
        ...
        try:
            ...
            if not dd_arrendatario.value: raise ValueError("Debe seleccionar un arrendatario")
    ...
    contenido = ft.Container(
        content=ft.Column([
            ...
            DocumentManager(entidad_tipo="CONTRATO_ARRENDAMIENTO", entidad_id=str(contrato_id), page=page) if es_edicion else ft.Text("Guarde el contrato para adjuntar documentos", color="grey", italic=True),
            
            ft.Divider(),
            ft.Row([
                ft.ElevatedButton("Cancelar", on_click=lambda e: on_cancelar(), style=styles.button_secondary()),
                ft.ElevatedButton("Guardar Arriendo", on_click=handle_guardar, style=styles.button_primary()),
