...
class LiquidacionesAsesoresListView(ft.Container):
        ...
        resumen_section = ft.Container(
            content=ft.ResponsiveRow(
                [
                    ...
                    self._crear_widget_resumen(
                        "Total Pagado", ft.Icons.PAYMENTS, "#4caf50", self.res_pagado, "$..."
                    ),
                ]
            ),
    ...
    def cargar_datos(self, mantener_opciones=False):
        ...
        threading.Thread(
            target=self._fetch_data_thread, args=(filtros, mantener_opciones), daemon=True
        ).start()
    ...
    def _fetch_data_thread(self, filtros, mantener_opciones):
        try:
            ...
            if not mantener_opciones:
                try:
                    asesores = self.servicio_personas.listar_personas(
                        filtro_rol="Asesor", solo_activos=False
                    )
                    opciones = []
            ...
            try:
                todos_asesores = self.servicio_personas.listar_personas(
                    filtro_rol="Asesor", solo_activos=False
                )
                cache_map = {}
            ...
            metricas = self.servicio.obtener_metricas_filtradas(
                id_asesor=id_asesor, periodo=filtros["periodo"] or None, estado=estado
            )
        ...
        if not self.data_liquidaciones:
                    ...
                    if liq["puede_editarse"]:
                        acciones.append(
                            ft.IconButton(
                                ...
                                on_click=lambda e, id=liq[
                                    "id_liquidacion_asesor"
                                ]: self.on_editar_liquidacion(id),
                            )
                        )
                    ...
                    if liq["puede_aprobarse"]:
                        acciones.append(
                            ft.IconButton(
                                ...
                                on_click=lambda e, id=liq["id_liquidacion_asesor"]: self.on_aprobar(
                                    id
                                ),
                            )
                        )
                    ...
                    if liq["puede_anularse"]:
                        acciones.append(
                            ft.IconButton(
                                ...
                                on_click=lambda e, id=liq["id_liquidacion_asesor"]: self.on_anular(
                                    id
                                ),
                            )
                        )
                        ...
                        cells=[
                            ...
                            ft.DataCell(
                                ft.Text(
                                    ...
                                    size=12,
                                    color="#f44336" if liq["total_descuentos"] > 0 else "#666",
                                )
                            ),
                            ft.DataCell(
                                ft.Text(
                                    f"${liq['valor_neto_asesor']:,}",
                                    size=13,
                                    weight=ft.FontWeight.BOLD,
                                )
                            ),
            ...
            tabla = ft.DataTable(
                columns=[
                    ...
                    ft.DataColumn(
                        ft.Text("% Cal", weight=ft.FontWeight.BOLD)
                    ),  # Header corto para espacio
...
def crear_liquidaciones_asesores_list_view(
    page,
    servicio_liquidacion_asesores,
    servicio_personas,
    on_nueva_liquidacion,
    on_editar_liquidacion,
    on_ver_detalle,
    on_aprobar,
    on_anular,
):
    return LiquidacionesAsesoresListView(
        page,
        servicio_liquidacion_asesores,
        servicio_personas,
        on_nueva_liquidacion,
