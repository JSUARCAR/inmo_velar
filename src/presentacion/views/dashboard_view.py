...
class DashboardView(ft.Container):
    def __init__(self, page: ft.Page, usuario, on_navigate=None):
        ...
        # Contenedores UI
        self.contenedor_kpis = ft.ResponsiveRow(run_spacing=20, spacing=20, alignment=ft.MainAxisAlignment.CENTER)
        ...
        self.main_content = ft.Column(
            [
                self.contenedor_kpis,
                self.contenedor_graficos_1,
                self.contenedor_graficos_2
            ],
            spacing=20,
        ...
        self.filtros = DashboardFilters(
            ...
            on_filter_apply=self.actualizar_dashboard_con_filtros
        )
    ...
    def _fetch_data_thread(self, mes, anio, id_asesor):
        try:
            ...
            contratos_activos = self.servicio_dashboard.obtener_total_contratos_activos(id_asesor=id_asesor)
            datos_comisiones = self.servicio_dashboard.obtener_comisiones_pendientes(id_asesor=id_asesor)
            ...
            datos_evolucion = self.servicio_dashboard.obtener_evolucion_recaudo(mes_fin=mes, anio_fin=anio)
    ...
    def _render_ui(self, data, error):
        if error:
            ...
            self.page_ref.snack_bar = ft.SnackBar(ft.Text(f"Error cargando dashboard: {error}"), bgcolor=colors.ERROR)
        ...
        fig_vencimientos = go.Figure(
            data=[
                go.Bar(name='30 días', x=['Vencimientos'], y=[data['vencimiento']['vence_30_dias']], marker_color='#EF4444'),
                go.Bar(name='60 días', x=['Vencimientos'], y=[data['vencimiento']['vence_60_dias']], marker_color='#F59E0B'),
                go.Bar(name='90 días', x=['Vencimientos'], y=[data['vencimiento']['vence_90_dias']], marker_color='#3B82F6')
            ]
        )
        fig_vencimientos.update_layout(barmode='stack', height=250, showlegend=True, margin=dict(l=20, r=20, t=20, b=20))
        self.contenedor_graficos_1.controls.append(
            ft.Container(
                content=ChartWidget("Contratos por Vencer", fig_vencimientos, "Proyección a 90 días"),
                col=chart_cols
            )
        )
        
        # Evolución
        fig_evolucion = go.Figure([go.Bar(
        ...
        fig_ocupacion.update_layout(height=250, showlegend=True, margin=dict(l=20, r=20, t=20, b=20))
        self.contenedor_graficos_2.controls.append(
            ft.Container(
                content=ChartWidget("Estado de Inventario", fig_ocupacion),
                col=chart_cols
            )
        )
        
        # Incidentes Pie
        fig_incidentes = go.Figure(data=[go.Pie(
            labels=list(data['incidentes']['por_estado'].keys()), 
            values=list(data['incidentes']['por_estado'].values()), 
            hole=.6
        )])
        fig_incidentes.update_layout(height=250, showlegend=True, margin=dict(l=20, r=20, t=20, b=20))
        self.contenedor_graficos_2.controls.append(
            ft.Container(
                content=ChartWidget("Incidentes por Estado", fig_incidentes),
                col=chart_cols
            )
        )
