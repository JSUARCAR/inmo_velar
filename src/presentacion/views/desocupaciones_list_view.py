    ...
    def __init__(
        ...
        self.content = ft.Column(
            [
                    ...
                    [
                        ...
                        ft.ElevatedButton(
                            ...
                            style=ft.ButtonStyle(
                                bgcolor=colors.PRIMARY, color=colors.TEXT_ON_PRIMARY, padding=20
                            ),
                            height=45,
                        ),
                    ]
                ...
                # Pagination
                ft.Container(content=self.pagination, padding=ft.padding.only(top=10)),
            ],
            spacing=10,
            expand=True,
        )  # Reduced spacing
    ...
    def _fetch_data(self, estado):
        try:
            result = self.servicio.listar_desocupaciones_paginado(
                page=self.current_page, page_size=self.page_size, estado=estado
            )
            self.desocupaciones = result.items
            self.total_items = result.total
            self._schedule_ui_update()
        except Exception as e:
            pass  # print(f"Error desocupaciones: {e}") [OpSec Removed]
    ...
    def _crear_tabla(self):
        ...
        for d in self.desocupaciones:
            ...
            progreso_col = ft.Column(
                [ft.Text(f"{d.progreso_porcentaje}%", size=12, weight="bold"), progreso_bar],
                spacing=4,
                horizontal_alignment=ft.CrossAxisAlignment.CENTER,
            )
            ...
            actions_menu = ft.PopupMenuButton(
                ...
                items=[
                    ...
                    ft.PopupMenuItem(
                        ...
                        on_click=lambda e, id=d.id_desocupacion: self._mostrar_gestor_documentos(
                            id
                        ),
                    ),
                ],
...
def crear_desocupaciones_list_view(
    page, servicio_desocupaciones, servicio_documental, on_nueva, on_ver_checklist
):
    return DesocupacionesListView(
        page, servicio_desocupaciones, servicio_documental, on_nueva, on_ver_checklist
    )
