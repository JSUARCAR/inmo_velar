...
def crear_persona_form_view(
    page: ft.Page, on_guardar: Callable, on_cancelar: Callable, persona_id: Optional[int] = None
) -> ft.Container:
    """
    ...
    txt_especialidad_prov = ft.TextField(
        label="Especialidad *", hint_text="Ej: Plomero, Electricista", width=250
    )

    txt_observaciones_prov = ft.TextField(
        label="Observaciones Proveedor", multiline=True, expand=True
    )
    ...
    chk_proveedor = ft.Checkbox(
        label="Proveedor", value=False, on_change=toggle_proveedor  # Default false
    )
    ...
    if es_edicion:
        ...
        if persona_prov:
            ...
            if not persona_actual.datos_roles:
                persona_actual.datos_roles = {}
    ...
    txt_banco_prop = ft.TextField(label="Banco", hint_text="Ej: Bancolombia", width=250)

    txt_cuenta_prop = ft.TextField(label="Número de Cuenta", hint_text="Ej: 987654321", width=250)
    ...
    txt_observaciones_prop = ft.TextField(
        label="Observaciones Propietario", multiline=True, min_lines=2, expand=True
    )
    ...
    txt_direccion_ref_arr = ft.TextField(
        label="Dirección Referencia *", hint_text="Dirección de trabajo o referencia", expand=True
    )
    ...
    drp_seguro_arr = ft.Dropdown(
        ...
        options=[
            ft.dropdown.Option(
                key=str(s.id_seguro), text=f"{s.nombre_seguro} ({s.obtener_porcentaje_decimal()}%)"
            )
    ...
    txt_cod_seguro_arr = ft.TextField(
        label="Cód. Aprobación Seguro", hint_text="Ej: ASEG-123456", width=250
    )
    ...
    txt_comision_arriendo = ft.TextField(
        label="% Com. Arriendo", value="0", width=150, suffix_text="%"
    )

    txt_comision_venta = ft.TextField(label="% Com. Venta", value="0", width=150, suffix_text="%")
    ...
    txt_observaciones_prov = ft.TextField(
        label="Observaciones Proveedor", multiline=True, min_lines=2, expand=True
    )
    ...
    chk_propietario = ft.Checkbox(
        label="Propietario", value="Propietario" in roles_actuales, on_change=toggle_propietario
    )

    chk_arrendatario = ft.Checkbox(
        label="Arrendatario", value="Arrendatario" in roles_actuales, on_change=toggle_arrendatario
    )

    chk_codeudor = ft.Checkbox(label="Codeudor", value="Codeudor" in roles_actuales)

    chk_asesor = ft.Checkbox(
        label="Asesor", value="Asesor" in roles_actuales, on_change=toggle_asesor
    )
    ...
    if es_edicion and persona_actual:
        ...
        if "Arrendatario" in roles_actuales:
            ...
            if arr:
                if arr.direccion_referencia:
                    txt_direccion_ref_arr.value = arr.direccion_referencia
                if arr.id_seguro:
                    drp_seguro_arr.value = str(arr.id_seguro)
                if arr.codigo_aprobacion_seguro:
                    txt_cod_seguro_arr.value = arr.codigo_aprobacion_seguro
        ...
        if "Asesor" in roles_actuales:
            ...
            if ase:
                if ase.fecha_ingreso:
                    txt_fecha_vinculacion.value = ase.fecha_ingreso
        ...
        if "Proveedor" in roles_actuales:
            ...
            if prov:
                if prov.especialidad:
                    txt_especialidad_prov.value = prov.especialidad
                if prov.observaciones:
                    txt_observaciones_prov.value = prov.observaciones

    # SECCIÓN 4: Roles (con campos dinámicos)
    section_roles = ft.Container(
        content=ft.Column(
            [
                ft.Text("ROLES", size=16, weight=ft.FontWeight.BOLD, color=colors.PRIMARY),
                ft.Divider(height=1, color=colors.BORDER_DEFAULT),
                ft.Row(
                    [chk_propietario, chk_arrendatario, chk_codeudor, chk_asesor, chk_proveedor],
                    spacing=30,
                ),
                ...
                ft.Text(
                    "Seleccione al menos un rol", size=12, color=colors.TEXT_SECONDARY, italic=True
                ),
            ],
            spacing=15,
        ),
    ...
    def handle_guardar_click(e):
        ...
        try:
            ...
            if es_edicion:
                # Actualizar persona
                persona_resultado = servicio.actualizar_persona(
                    persona_id, datos_persona, usuario_sistema="admin"
                )
                ...
                for rol in roles_nuevos.intersection(roles_previos):
                    ...
                    if extra:
                        servicio.actualizar_datos_rol(
                            persona_id, rol, extra, usuario_sistema="admin"
                        )
            ...
            if "Proveedor" in roles_seleccionados:
                ...
                if prov_existente:
                    servicio_proveedores.actualizar_proveedor(
                        prov_existente.id_proveedor, datos_prov
                    )
                else:
            ...
            [
                ...
                ft.Container(
                    content=ft.Column(
                        [
                            ft.Text(
                                f"Inicio > Personas > {'Editar' if es_edicion else 'Nueva'}",
                ...
                ft.Container(
                    content=ft.Column(
                        [
                            ...
                            ft.Divider(height=1, color=colors.BORDER_DEFAULT),
                            ft.Row([txt_tipo_doc, txt_numero_doc], spacing=20),
                            txt_nombre_completo,
                        ],
                        spacing=15,
                    ),
                ...
                ft.Container(
                    content=ft.Column(
                        [
                            ft.Text(
                                "CONTACTO", size=16, weight=ft.FontWeight.BOLD, color=colors.PRIMARY
                            ),
                            ft.Divider(height=1, color=colors.BORDER_DEFAULT),
                            ft.Row([txt_celular, txt_correo], spacing=20),
                        ],
                        spacing=15,
                    ),
                    padding=ft.padding.only(bottom=20),
