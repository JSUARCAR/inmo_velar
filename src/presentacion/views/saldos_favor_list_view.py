...
class SaldosFavorListView(ft.Container):
    def __init__(
        ...
        self.propietarios_dict = {p.get('id_propietario'): p.get('nombre', 'Desc') for p in (propietarios_data or [])}
        self.asesores_dict = {a.get('id_asesor'): a.get('nombre', 'Desc') for a in (asesores_data or [])}
        ...
        filtros = ft.Row([
            ft.Dropdown(
                ref=self.dd_tipo, label="Tipo Beneficiario", width=200,
                options=[ft.dropdown.Option("", "Todos"), ft.dropdown.Option("Propietario", "Propietario"), ft.dropdown.Option("Asesor", "Asesor")],
                on_change=lambda e: self.cargar_datos()
            ),
            ft.Dropdown(
                ref=self.dd_estado, label="Estado", width=180,
                options=[ft.dropdown.Option("", "Todos"), ft.dropdown.Option("Pendiente", "Pendiente"), ft.dropdown.Option("Aplicado", "Aplicado"), ft.dropdown.Option("Devuelto", "Devuelto")],
                on_change=lambda e: self.cargar_datos()
            ),
        ...
        self.content = ft.Column([
            ft.Row([
                ft.Text("Saldos a Favor", size=28, weight="bold"), ft.Container(expand=True),
                ft.ElevatedButton("Nuevo Saldo", icon=ft.Icons.ADD, on_click=lambda e: self.on_nuevo(), style=ft.ButtonStyle(bgcolor=ft.Colors.BLUE_600, color=ft.Colors.WHITE))
            ]),
            ft.Divider(height=20),
            
            # Resumen
            ft.Row([
                ft.Container(content=ft.Column([ft.Text("Total Pendiente", color="grey", size=13), ft.Text(ref=self.txt_pend, value="$0", size=24, weight="bold", color="orange")], spacing=4),
                    bgcolor=ft.Colors.ORANGE_50, padding=20, border_radius=12, expand=True),
                ft.Container(content=ft.Column([ft.Text("Total Resuelto", color="grey", size=13), ft.Text(ref=self.txt_res, value="$0", size=24, weight="bold", color="green")], spacing=4),
                    bgcolor=ft.Colors.GREEN_50, padding=20, border_radius=12, expand=True),
            ], spacing=16),
    ...
    def cargar_datos(self):
        ...
        filtros = {
            'tipo': self.dd_tipo.current.value,
            'estado': self.dd_estado.current.value
        }
    ...
    def _fetch_data(self, filtros):
        try:
            saldos = self.servicio.listar_saldos(
                tipo_beneficiario=filtros['tipo'] or None,
    ...
    def _crear_tabla(self):
        ...
        for s in self.saldos:
            ...
            if s.esta_pendiente:
                acciones.extend([
                    ft.IconButton(ft.Icons.CHECK_CIRCLE_OUTLINE, icon_color="green", tooltip="Aplicar", on_click=lambda e, id=s.id_saldo_favor: self.on_aplicar(id)),
                    ft.IconButton(ft.Icons.REPLY, icon_color="blue", tooltip="Devolver", on_click=lambda e, id=s.id_saldo_favor: self.on_devolver(id)),
                    ft.IconButton(ft.Icons.DELETE_OUTLINE, icon_color="red", tooltip="Eliminar", on_click=lambda e, id=s.id_saldo_favor: self.on_eliminar(id)),
                ])
            
            rows.append(ft.DataRow(cells=[
                ft.DataCell(ft.Text(str(s.id_saldo_favor))),
                ft.DataCell(ft.Text(nombre, max_lines=1)),
                ft.DataCell(ft.Row([ft.Icon(ft.Icons.PERSON if s.tipo_beneficiario == 'Propietario' else ft.Icons.SUPPORT_AGENT, size=16), ft.Text(s.tipo_beneficiario)], spacing=4)),
                ft.DataCell(ft.Text(f"${s.valor_saldo:,.0f}", weight="bold")),
                ft.DataCell(ft.Text(s.motivo[:30] + "..." if len(s.motivo) > 30 else s.motivo, size=12)),
        ...
        return ft.Column([
            ft.DataTable(
                columns=[
                    ft.DataColumn(ft.Text("ID")), ft.DataColumn(ft.Text("Beneficiario")),
                    ft.DataColumn(ft.Text("Tipo")), ft.DataColumn(ft.Text("Valor")),
                    ft.DataColumn(ft.Text("Motivo")), ft.DataColumn(ft.Text("Fecha")),
    ...
    def get_nombre_beneficiario(self, saldo):
        if saldo.tipo_beneficiario == 'Propietario':
            return self.propietarios_dict.get(saldo.id_propietario, f"Prop. #{saldo.id_propietario}")
        else:
            return self.asesores_dict.get(saldo.id_asesor, f"Asesor #{saldo.id_asesor}")

    def get_estado_chip(self, estado):
        colors = {'Pendiente':('orange', 'orange'), 'Aplicado':('green', 'green'), 'Devuelto':('blue', 'blue')}
        bg, fg = colors.get(estado, ('grey', 'grey'))
        return ft.Container(content=ft.Text(estado, color=fg.replace("100", "800"), size=12), bgcolor=bg.replace("800", "100"), padding=5, border_radius=12)

def build_saldos_favor_list_view(page, servicio_saldos_favor, on_nuevo, on_aplicar, on_devolver, on_eliminar, propietarios_data=None, asesores_data=None):
    return SaldosFavorListView(page, servicio_saldos_favor, on_nuevo, on_aplicar, on_devolver, on_eliminar, propietarios_data, asesores_data)
