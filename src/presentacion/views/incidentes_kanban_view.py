...
class IncidentesKanbanView(ft.Column):
    ...
    def __init__(
        ...
        servicio_propiedades: ServicioPropiedades,
    ...
    def _crear_panel_filtros(self) -> ft.Container:
        ...
        filtros_row = ft.Row([self.txt_busqueda, self.dd_propiedad, self.dd_prioridad], spacing=10)
    ...
    def _crear_tablero_kanban(self) -> ft.Container:
        ...
        board = ft.Row(
            [col_reportado, col_cotizado, col_aprobado, col_reparacion, col_finalizado],
            spacing=15,
            expand=True,
            scroll=ft.ScrollMode.AUTO,
        )

        return ft.Container(content=board, padding=20, expand=True)
    ...
    def _crear_columna(self, titulo: str, incidentes: List, color: str) -> ft.Container:
        ...
        if not tarjetas:
            tarjetas = [
                ft.Container(
                    content=ft.Text(
                        "No hay incidentes", size=12, color=ft.Colors.GREY_400, italic=True
                    ),
                    padding=20,
        ...
        contenido_columna = ft.Column(
            controls=tarjetas, scroll=ft.ScrollMode.AUTO, spacing=10, expand=True
        )

        return ft.Container(
            content=ft.Column(
                [header, ft.Container(content=contenido_columna, padding=10, expand=True)],
                spacing=0,
            ),
            width=280,
    ...
    def _crear_tarjeta_incidente(self, incidente, color_columna) -> ft.Container:
        ...
        icono_prioridad, color_prioridad = prioridad_config.get(
            incidente.prioridad, ("⚪", ft.Colors.GREY_400)
        )

        # Dirección truncada
        direccion = (
            incidente.direccion_propiedad
            if hasattr(incidente, "direccion_propiedad") and incidente.direccion_propiedad
            else f"Propiedad #{incidente.id_propiedad}"
        )
        ...
        tarjeta = ft.Container(
            content=ft.Column(
                [
                    ...
                    ft.Row(
                        [
                            ...
                            ft.IconButton(
                                ...
                                on_click=lambda _, id=incidente.id_incidente: self.on_navigate(
                                    "incidente_detalle", id_incidente=id
                                ),
                            ),
                        ],
                        alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                    ...
                    ft.Container(
                        content=ft.Text(
                            f"⚡ {incidente.prioridad}", size=11, color=ft.Colors.WHITE
                        ),
                    ...
                    ft.Row(
                        [
                            ...
                            ft.Text(
                                f"{dias} días", size=11, color=dias_color, weight=ft.FontWeight.BOLD
                            ),
                        ],
                        spacing=5,
                    ),
    ...
    def aplicar_filtros(self, e):
        ...
        self.filtro_propiedad = (
            int(self.dd_propiedad.value) if self.dd_propiedad.value != "0" else None
        )
        self.filtro_prioridad = (
            self.dd_prioridad.value if self.dd_prioridad.value != "Todas" else None
        )
