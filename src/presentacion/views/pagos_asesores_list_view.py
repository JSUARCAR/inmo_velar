...
class PagosAsesoresListView(ft.Container):
            ...
            content=ft.ResponsiveRow(
                [
                    self._crear_card_resumen(
                        "Por Pagar", ft.Icons.PENDING_ACTIONS, "#ff9800", self.resumen_pendiente
                    ),
                    self._crear_card_resumen(
                        "Liquidaciones Aprobadas",
                        ft.Icons.SCHEDULE,
                        "#2196f3",
                        self.resumen_programado,
                        is_count=True,
                    ),
                    self._crear_card_resumen(
                        "Total Pagado", ft.Icons.CHECK_CIRCLE, "#4caf50", self.resumen_pagado
                    ),
                ]
            ),
            ...
            [
                ft.Row(
                    [
                        ...
                        ft.Text("Pagos Asesores", size=14, weight="bold", color="#1976d2"),
                    ]
                ),
    ...
    def _fetch_data(self, filtros, mantener_opciones):
        try:
            ...
            if not mantener_opciones:
                try:
                    ...
                    for a in asesores:
                        opciones.append(
                            ft.dropdown.Option(
                                key=str(a.persona.id_persona), text=a.nombre_completo
                            )
                        )
            ...
            if estado and estado != "Todos":
                ...
                if estado == "Pendiente":
                    liquidaciones = [
                        l for l in liquidaciones if l["estado_liquidacion"] == "Aprobada"
                    ]
                elif estado == "Pagado":
                    liquidaciones = [
                        l for l in liquidaciones if l["estado_liquidacion"] == "Pagada"
                    ]
            ...
            total_pend = sum(
                ...
                if l["estado_liquidacion"] == "Aprobada"
            )
            total_pagd = sum(
                l["valor_neto_asesor"] for l in liquidaciones if l["estado_liquidacion"] == "Pagada"
            )
    ...
    def _crear_tabla(self):
        if not self.pagos:
            return ft.Container(
                content=ft.Text("No hay pagos pendientes", size=16),
                padding=50,
        ...
        for liq in self.pagos:
            ...
            color = {"Pendiente": "#ff9800", "Pagado": "#4caf50", "Aprobada": "#2196f3"}.get(
                liq["estado_liquidacion"], "grey"
            )
            ...
            if liq["estado_liquidacion"] == "Aprobada":
                acciones.append(
                    ft.ElevatedButton(
                        ...
                        color="white",
                        height=35,
                        on_click=lambda e, id=liq["id_liquidacion_asesor"]: self.on_registrar_pago(
                            id
                        ),
                    )
                )
                acciones.append(
                    ft.IconButton(
                        ...
                        icon_color="#f44336",
                        tooltip="Rechazar",
                        on_click=lambda e, id=liq["id_liquidacion_asesor"]: self.on_rechazar_pago(
                            id
                        ),
                    )
                )
            ...
            filas.append(
                ft.DataRow(
                    cells=[
                        ...
                        ft.DataCell(
                            ft.Text(
                                f"${liq['valor_neto_asesor']:,}", weight="bold", color="#4caf50"
                            )
                        ),
                        ft.DataCell(
                            ft.Container(
                                content=ft.Text(liq["estado_liquidacion"], color="white", size=11),
                                bgcolor=color,
                                padding=5,
                                border_radius=4,
                            )
        ...
        return ft.Container(
            content=ft.DataTable(
                columns=[
                    ...
                    ft.DataColumn(ft.Text("Asesor")),
                    ft.DataColumn(ft.Text("Contrato")),
                    ft.DataColumn(ft.Text("Valor a Pagar")),
...
def crear_pagos_asesores_list_view(
    page,
    servicio_liquidacion_asesores,
    servicio_personas,
    on_registrar_pago,
