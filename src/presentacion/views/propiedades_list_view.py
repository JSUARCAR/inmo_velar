...
class PropiedadesListView(ft.Container):
            ...
            [
                ...
                ft.Container(
                    content=ft.Column([
                        ...
                        ft.Text("Gestión de Propiedades", size=24, weight=ft.FontWeight.BOLD, color=colors.TEXT_PRIMARY),
                    ]),
    ...
    def toggle_vista(self, e):
        ...
        self.btn_toggle_vista.icon = ft.Icons.VIEW_MODULE if self.vista_tipo == "table" else ft.Icons.TABLE_ROWS
        self.btn_toggle_vista.tooltip = "Ver como Tabla" if self.vista_tipo == "cards" else "Ver como Tarjetas"
    ...
    def cargar_datos(self):
        ...
        filtros = {
            ...
            'busqueda': self.txt_busqueda.current.value.strip() if self.txt_busqueda.current and self.txt_busqueda.current.value else None,
        ...
        if error:
            ...
            self.contenedor_propiedades.content = ft.Container(
                content=ft.Column([
                    ...
                    ft.Text("No se encontraron propiedades", size=16, color=colors.TEXT_SECONDARY),
                ], alignment=ft.MainAxisAlignment.CENTER),
                alignment=ft.alignment.center,
                expand=True
            )
            ...
            if elapsed > 0:
                ...
                self.txt_contador.value = f"Mostrando {start_idx}-{end_idx} de {self.total_items} propiedades"
    ...
    def handle_toggle_disponibilidad(self, id_propiedad: int, nueva_disponibilidad: int):
        try:
            self.servicio.cambiar_disponibilidad(id_propiedad, nueva_disponibilidad, usuario_sistema="admin")
            estado = "Disponible" if nueva_disponibilidad == 1 else "Ocupada"
            
            if self.page_ref:
                self.page_ref.snack_bar = ft.SnackBar(ft.Text(f"Propiedad marcada como {estado}"), bgcolor=colors.SUCCESS)
