...
class SegurosListView(ft.Container):
    def __init__(
        self,
        page: ft.Page,
        on_nuevo_seguro: Callable,
        on_editar_seguro: Callable[[int], None],
        on_nueva_poliza: Callable = None,
    ):
        ...
        return ft.Row(
            [
                ft.TextField(
                    ...
                    hint_text="Buscar seguro...",
                    expand=True,
                    on_submit=lambda e: self.cargar_datos(),
                ),
                ft.IconButton(ft.Icons.SEARCH, on_click=lambda e: self.cargar_datos()),
                ft.Switch(
                    ref=self.switch_activos,
                    label="Solo activos",
                    value=True,
                    on_change=lambda e: self.cargar_datos(),
                ),
                ft.ElevatedButton(
                    "Nuevo Seguro",
                    icon=ft.Icons.ADD,
                    on_click=lambda e: self.on_nuevo_seguro(),
        ...
        return ft.Row(
            [
                ...
                ft.ElevatedButton(
                    ...
                    on_click=lambda e: self.on_nueva_poliza(),
                    bgcolor=colors.PRIMARY,
                    color="white",
                ),
            ],
    ...
    def _update_ui(self):
        ...
        for s in self.seguros:
            self.tabla_seguros.rows.append(
                ft.DataRow(
                    cells=[
                            ...
                            ft.Row(
                                [
                                    ft.IconButton(
                                        ...
                                        on_click=lambda e, id=s.id_seguro: self.on_editar_seguro(
                                            id
                                        ),
                                    ),
                                    ft.IconButton(
                                        ...
                                        on_click=lambda e, obj=s: self.handle_toggle_estado_seguro(
                                            obj
                                        ),
                                    ),
                                ]
        ...
        if self.tabs.selected_index == 1:
            for p in self.polizas:
                self.tabla_polizas.rows.append(
                    ft.DataRow(
                        cells=[
                                ...
                                ft.Column(
                                    [
                                        ...
                                        ft.Text(
                                            ...
                                            color=(
                                                "red"
                                                if p.fecha_fin < datetime.now().strftime("%Y-%m-%d")
                                                else "black"
                                            ),
                                        ),
                                    ]
