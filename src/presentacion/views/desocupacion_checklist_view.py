...
class DesocupacionChecklistView(ft.Container):
            ...
            ft.Container(
                content=ft.Column([
                    ft.Row([
                        ...
                        ft.Text(ref=self.progress_text_ref, value="0/8", size=16, weight="bold")
                    ]),
                    ft.ProgressBar(
    ...
    def _schedule_ui_update(self, error=None):
        ...
        if self.progreso_data:
            ...
            if self.progress_text_ref.current:
                self.progress_text_ref.current.value = f"{self.progreso_data['completadas']}/{self.progreso_data['total']}"
                self.progress_text_ref.current.update()
            
            # Enable/disable finalizar button
            if self.btn_finalizar_ref.current:
                self.btn_finalizar_ref.current.disabled = not self.progreso_data['puede_finalizar'] or self.desocupacion.estado != "En Proceso"
        ...
        return ft.Container(
            content=ft.Column([
                ft.Row([
                    ...
                    ft.Text(f"Propiedad: {self.desocupacion.direccion_propiedad}", size=16, weight="bold")
                ], spacing=8),
                ft.Row([
                ...
                ft.Row([
                    ...
                    ft.Text(f"Fecha Programada: {self.desocupacion.fecha_programada}", size=16)
                ], spacing=8)
            ], spacing=8),
    ...
    def _build_checklist(self):
        ...
        for tarea in self.tareas:
            ...
            if tarea.completada:
                ...
                status_icon = ft.Icon(ft.Icons.RADIO_BUTTON_UNCHECKED, color=ft.Colors.GREY, size=20)
            ...
            tarea_container = ft.Container(
                ...
                padding=16,
                border_radius=8,
                border=ft.border.all(1, ft.Colors.GREEN_200 if tarea.completada else ft.Colors.GREY_200)
            )
    ...
    def _handle_finalizar(self, e):
        ...
        def confirmar(e):
            try:
                self.servicio.finalizar_desocupacion(self.id_desocupacion, "admin")  # TODO: Get from session
                self.page_ref.close(dlg)
                self.page_ref.snack_bar = ft.SnackBar(
                    content=ft.Text("Desocupación finalizada exitosamente. Contrato marcado como Finalizado."),
                    bgcolor="green"
                )
                self.page_ref.snack_bar.open = True
                self.page_ref.update()
        ...
        dlg = ft.AlertDialog(
            ...
            actions=[
                ft.TextButton("Cancelar", on_click=lambda e: self.page_ref.close(dlg)),
                ft.TextButton("Finalizar", on_click=confirmar, style=ft.ButtonStyle(color=ft.Colors.GREEN))
            ]
        )
        self.page_ref.open(dlg)
