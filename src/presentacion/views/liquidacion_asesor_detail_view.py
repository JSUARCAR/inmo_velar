...
def crear_modal_detalle_liquidacion(
    ...
    try:
        ...
        info_general = ft.Container(
            content=ft.Column([
                ft.Text("Información General", size=16, weight=ft.FontWeight.BOLD, color="#1976d2"),
                ft.Divider(height=1),
                ft.Row([
        ...
        if detalle.get('propiedad'):
            ...
            info_predio = ft.Container(
                content=ft.Column([
                    ft.Text("Información del Predio", size=16, weight=ft.FontWeight.BOLD, color="#1976d2"),
                    ft.Divider(height=1),
                    ft.Row([
                        ft.Text("Contrato ID:", weight=ft.FontWeight.BOLD, size=13),
        ...
        calculos = ft.Container(
            content=ft.Column([
                ft.Text("Cálculo de Comisión", size=16, weight=ft.FontWeight.BOLD, color="#1976d2"),
                ft.Divider(height=1),
                ft.Row([
                    ft.Text("Canon Liquidado:", size=13),
                    ft.Text(f"${liquidacion['canon_arrendamiento_liquidado']:,}", size=13, weight=ft.FontWeight.BOLD),
                ...
                ft.Row([
                    ...
                    ft.Text(f"${liquidacion['comision_bruta']:,}", size=13, color="#1976d2"),
                ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                ft.Row([
                    ft.Text("Total Descuentos:", size=13),
                    ft.Text(f"${liquidacion['total_descuentos']:,}", size=13, color="#f44336"),
                ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                ft.Divider(height=1),
                ft.Row([
        ...
        if contratos_asociados:
            ...
            for contrato in contratos_asociados:
                ...
                comision_individual = int(contrato['canon_incluido'] * liquidacion['porcentaje_real'] / 100)
                
                filas_contratos.append(
                    ft.DataRow(cells=[
                        ft.DataCell(ft.Text(f"#{contrato['id_contrato']}", size=12, weight=ft.FontWeight.BOLD)),
                        ft.DataCell(ft.Text(contrato['direccion'], size=11)),
                        ft.DataCell(ft.Text(f"${contrato['canon_incluido']:,}", size=12)),
                        ft.DataCell(ft.Text(f"${comision_individual:,}", size=12, color="#1976d2")),
                    ])
                )
            ...
            tabla_contratos = ft.DataTable(
                columns=[
                    ...
                    ft.DataColumn(ft.Text("Dirección Propiedad", size=12, weight=ft.FontWeight.BOLD)),
            ...
            seccion_descuentos = ft.Container(
                content=ft.Column([
                    ft.Row([
                        ...
                        ft.Text("Egresos y Deducciones Aplicados", size=14, weight=ft.FontWeight.BOLD, color="#f44336"),
                    ]),
                    tabla_descuentos,
                    ft.Container(
                        content=ft.Text(
                    ...
                    ) if liquidacion['estado_liquidacion'] == 'Pendiente' else ft.Container(),
                ], spacing=10),
                padding=ft.padding.only(top=15),
            ...
            seccion_descuentos = ft.Container(
                content=ft.Column([
                    ft.Row([
                        ...
                        ft.Text("Egresos y Deducciones", size=14, weight=ft.FontWeight.BOLD, color="#666"),
                    ]),
                    ft.Container(
                        content=ft.Row([
                            ft.Icon(ft.Icons.CHECK_CIRCLE_OUTLINE, color="#4caf50", size=16),
                            ft.Text("Sin descuentos aplicados - Valor neto = Comisión bruta", size=12, color="#666", italic=True),
                        ]),
        ...
        contenido = ft.Container(
            content=ft.Column([
                ...
                DocumentManager(entidad_tipo="LIQUIDACION", entidad_id=str(id_liquidacion), page=page, height=250),
        ...
        contenido = ft.Container(
            content=ft.Text(f"Error al cargar detalles: {str(e)}", color="red"),
            padding=20
        )
    
    def handle_enviar_email(e):
        page.snack_bar = ft.SnackBar(ft.Text("⏳ Enviando correo... Esto puede tardar unos segundos."))
        ...
        try:
            ...
            def continuar_envio_whatsapp():
                ...
                try:
                    ...
                    if exito:
                        page.snack_bar = ft.SnackBar(
                            ft.Text(f"✅ WhatsApp abierto para {nombre_asesor}. El mensaje se enviará automáticamente."), 
                            bgcolor="green",
                            duration=5000
                        )
                    else:
                        page.snack_bar = ft.SnackBar(
                            ft.Text("⚠️ No se pudo abrir WhatsApp. Verifique que WhatsApp Desktop esté instalado."), 
                            bgcolor="orange",
                            duration=5000
                        )
            ...
            if not telefono:
                ...
                def confirmar_envio_whatsapp_manual(ev):
                    ...
                    if not telefono_input or len(telefono_input) < 10:
                        txt_telefono_manual.error_text = "Ingrese un número válido (mínimo 10 dígitos)"
                        page.update()
                        return
                ...
                dlg_telefono_manual = ft.AlertDialog(
                    ...
                    content=ft.Column([
                        ...
                        txt_telefono_manual,
                        ft.Text("Nota: Debería actualizar este campo en 'Personas > Editar'", size=11, color="orange")
                    ], tight=True, spacing=10),
                    actions=[
                        ft.TextButton("Cancelar", on_click=lambda e: page.close(dlg_telefono_manual)),
    ...
    def handle_descargar_pdf(e):
        try:
            ...
            page.snack_bar = ft.SnackBar(ft.Text(f"✅ PDF generado correctamente"), bgcolor="green")
            page.snack_bar.open = True
            page.update()
        except Exception as ex:
    ...
    dialog = ft.AlertDialog(
        ...
        content=contenido,
        actions=[
            ft.TextButton("Descargar PDF", icon=ft.Icons.PICTURE_AS_PDF, on_click=lambda e: handle_descargar_pdf(e)),
