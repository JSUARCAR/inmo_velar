...
class IncrementosView(ft.Container):
                ...
                ft.Container(
                    content=ft.TextField(
                        ...
                        suffix=ft.IconButton(
                            ...
                            on_click=lambda e: self.page_ref.open(self.dp_inicio),
                        ),
                    )
                ),
                ft.Container(
                    content=ft.TextField(
                        ...
                        suffix=ft.IconButton(
                            ft.Icons.CALENDAR_MONTH,
                            on_click=lambda e: self.page_ref.open(self.dp_fin),
                        ),
                    )
                ),
                ft.ElevatedButton(
                    "Proyectar", icon=ft.Icons.SEARCH, on_click=lambda e: self.cargar_proyeccion()
                ),
                ft.VerticalDivider(),
                ft.ElevatedButton(
            ...
            rows.append(
                ft.DataRow(
                    cells=[
                        ...
                        ft.DataCell(
                            ft.Text(f"{item['ipc_porcentaje']}%", color="blue", weight="bold")
                        ),
                        ft.DataCell(ft.Text(f"${item['canon_proyectado']:,.0f}", weight="bold")),
                        ft.DataCell(
                            ft.Text(f"+${item['incremento_valor']:,.0f}", color="green", size=11)
                        ),
                    ]
                )
            )
        ...
        if not self.resultados:
            rows.append(
                ft.DataRow(
                    cells=[
                        ft.DataCell(
                            ft.Text("No se encontraron contratos venciendo en este periodo.")
                        )
                        for _ in range(8)
                    ]
                )
    ...
    def aplicar_incrementos(self, e):
        ...
        dlg = ft.AlertDialog(
            ...
            content=ft.Text(
                f"¿Está seguro de renovar {len(self.seleccionados)} contratos aplicando el IPC actual?"
            ),
            actions=[
                ft.TextButton("Cancelar", on_click=lambda e: self.page_ref.close(dlg)),
                ft.ElevatedButton(
                    "Confirmar", on_click=confirm_action, bgcolor="green", color="white"
                ),
            ],
        )
