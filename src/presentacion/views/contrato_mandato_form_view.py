...
def crear_contrato_mandato_form_view(
    ...
    propiedad_canon_map = {str(p["id"]): p["canon"] for p in propiedades}
    ...
    dd_propiedad = ft.Dropdown(
        ...
        options=[
            ft.dropdown.Option(key=str(p["id"]), text=p["texto"]) 
    ...
    def on_fecha_inicio_change(e):
        txt_fecha_inicio.value = date_picker_inicio.value.strftime("%Y-%m-%d") if date_picker_inicio.value else ""
        txt_fecha_inicio.update()
        calcular_duracion()

    def on_fecha_fin_change(e):
        txt_fecha_fin.value = date_picker_fin.value.strftime("%Y-%m-%d") if date_picker_fin.value else ""
    ...
    txt_comision = ft.TextField(label="Comisión (%) *", suffix_text="%", width=150, keyboard_type=ft.KeyboardType.NUMBER)
    ...
    if es_edicion:
        ...
        if mandato:
            ...
            if not val_exists:
                dd_propiedad.options.append(ft.dropdown.Option(key=str(mandato.id_propiedad), text=f"Propiedad Actual (ID: {mandato.id_propiedad})"))
                dd_propiedad.value = str(mandato.id_propiedad)
            ...
            for p in propietarios:
                ...
                if role and role.id_propietario == mandato.id_propietario:
                    ...
                    dd_propietario.disabled = True # No modificar propietario (vinculado a propiedad)
                    break
    ...
    def show_banner(message: str, is_error: bool = True):
        page.banner = ft.Banner(
            ...
            leading=ft.Icon(ft.Icons.ERROR_OUTLINE if is_error else ft.Icons.CHECK_CIRCLE, color=colors.TEXT_PRIMARY),
    ...
    def handle_guardar(e):
        ...
        try:
            ...
            if not dd_propietario.value: raise ValueError("Debe seleccionar un propietario")
            if not dd_asesor.value: raise ValueError("Debe seleccionar un asesor")
            ...
            datos = {
                ...
                "id_propietario": int(dd_propietario.value), # ID Persona (Servicio resolverá o ya está resuelto?)
                # OJO: crear_mandato espera ID_PROPIETARIO, pero aquí enviamos ID_PERSONA del dropdown.
                # PERO `crear_mandato` recibe lo que enviamos.
                "id_propietario": int(dd_propietario.value),
            ...
            prop_obj = next((p for p in propietarios if p.persona.id_persona == id_persona_propietario), None)
            ...
            asesor_obj = next((a for a in asesores if a.persona.id_persona == id_persona_asesor), None)
    ...
    contenido = ft.Container(
        content=ft.Column([
            ...
            ft.Row([
                txt_fecha_inicio, btn_fecha_inicio,
                txt_fecha_fin, btn_fecha_fin,
                txt_duracion
            ...
            DocumentManager(entidad_tipo="CONTRATO_MANDATO", entidad_id=str(contrato_id), page=page) if es_edicion else ft.Text("Guarde el contrato para adjuntar documentos", color="grey", italic=True),
            
            ft.Divider(),
            ft.Row([
                ft.ElevatedButton("Cancelar", on_click=lambda e: on_cancelar(), style=styles.button_secondary()),
                ft.ElevatedButton("Guardar Mandato", on_click=handle_guardar, style=styles.button_primary()),
