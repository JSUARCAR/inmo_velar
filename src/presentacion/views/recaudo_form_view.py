    ...
    def handle_guardar(e):
        ...
        try:
            ...
            datos_recaudo = {
                ...
                "referencia_bancaria": (
                    txt_referencia.current.value
                    if dd_metodo_pago.current.value != "Efectivo"
                    else None
                ),
            ...
            recaudo_guardado = servicio_financiero.registrar_recaudo(
                datos_recaudo, conceptos_list, "admin"
            )
            ...
            page.snack_bar = ft.SnackBar(
                content=ft.Text(
                    f"✅ Recaudo #{recaudo_guardado.id_recaudo} registrado exitosamente"
                ),
            ...
            page.snack_bar = ft.SnackBar(
                content=ft.Text(f"❌ {str(ve)}"), bgcolor=ft.Colors.RED, duration=5000
            )
            page.snack_bar.open = True
            page.update()
