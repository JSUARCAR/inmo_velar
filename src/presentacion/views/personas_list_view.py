...
class PersonasListView(ft.Container):
    def __init__(
        self, page: ft.Page, on_nueva_persona: Callable, on_editar_persona: Callable[[int], None]
    ):
        ...
        self.search_debouncer = Debouncer(
            delay_ms=500, callback=self._on_search_debounced, name="personas_search"
        )
    ...
    def _actualizar_tabla(self, elapsed, error):
        ...
        if error:
            ...
            self.txt_contador.value = (
                f"Mostrando {start_idx}-{end_idx} de {self.total_items} personas ({elapsed:.3f}s)"
            )
    ...
    def handle_toggle_estado(self, id_persona: int, nuevo_estado: bool):
        ...
        try:
            if nuevo_estado:
                ...
                self.servicio.desactivar_persona(
                    id_persona, motivo="UI Switch", usuario_sistema="admin"
                )
