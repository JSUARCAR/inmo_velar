...
class Sidebar(ft.Container):
    ...
    def __init__(self, usuario, on_navigate, on_logout, collapsed=False, on_toggle=None, responsive=False, page_width=None):
        super().__init__()
        ...
        if self.collapsed:
            ...
            header_content = ft.Row(
                [
                    ...
                    ft.Column(
                        [
                            ft.Text("VELAR", weight=ft.FontWeight.BOLD, size=18, color=colors.TEXT_PRIMARY),
        ...
        if self.collapsed:
            ...
            profile_content = ft.Column(
                [
                    ...
                    ft.Text(self.usuario.nombre_usuario, weight=ft.FontWeight.BOLD, color=colors.TEXT_PRIMARY),
                    styles.badge_admin() if self.usuario.rol == "Administrador" else styles.badge_asesor()
                ],
                horizontal_alignment=ft.CrossAxisAlignment.CENTER
            )
        ...
        return ft.Column(
            [
                ...
                ft.Container(
                    ...
                    padding=ft.padding.symmetric(vertical=20, horizontal=10 if self.collapsed else 20),
                ...
                ft.Column(
                    [
                        # Operaciones
                        ft.Container(
                            content=ft.Text("OPS", size=10, color=colors.TEXT_SECONDARY, weight=ft.FontWeight.BOLD) if self.collapsed else ft.Text("OPERACIONES", size=10, color=colors.TEXT_SECONDARY, weight=ft.FontWeight.BOLD),
                            padding=ft.padding.only(left=5 if self.collapsed else 0),
                            alignment=ft.alignment.center if self.collapsed else ft.alignment.center_left
                        ),
                        
                        self._menu_item("Dashboard", ft.Icons.DASHBOARD_OUTLINED, route="dashboard"),
                        self._menu_item("Personas", ft.Icons.PEOPLE_OUTLINE, route="personas"),
                        self._menu_item("Proveedores", ft.Icons.ENGINEERING_OUTLINED, route="proveedores"),
                        self._menu_item("Seguros", ft.Icons.SHIELD_OUTLINED, route="seguros"),
                        self._menu_item("Propiedades", ft.Icons.HOME_WORK_OUTLINED, route="propiedades"),
                        self._menu_item("Desocupaciones", ft.Icons.NO_MEETING_ROOM_OUTLINED, route="desocupaciones"),
                        self._menu_item("Contratos", ft.Icons.DESCRIPTION_OUTLINED, route="contratos"),
                        self._menu_item("Recaudos", ft.Icons.ATTACH_MONEY, route="recaudos"),
                        self._menu_item("Liquidaciones", ft.Icons.ACCOUNT_BALANCE_WALLET, route="liquidaciones"),
                        self._menu_item("Liquidación Asesores", ft.Icons.HANDSHAKE_OUTLINED, route="liquidaciones_asesores"),
                        self._menu_item("Pagos Asesores", ft.Icons.PAYMENTS, route="pagos_asesores"),
                        self._menu_item("Saldos a Favor", ft.Icons.ACCOUNT_BALANCE_WALLET_OUTLINED, route="saldos_favor"),
                        self._menu_item("Recibos Públicos", ft.Icons.RECEIPT_LONG, route="recibos_publicos"),
                        self._menu_item("Incrementos IPC", ft.Icons.TRENDING_UP, route="incrementos"),
                        ...
                        ft.Container(
                            content=ft.Column([
                                ft.Divider(height=1, color=colors.BORDER_DEFAULT),
                                ft.Container(
                                    content=ft.Icon(ft.Icons.SETTINGS, size=16, color=colors.TEXT_SECONDARY) if self.collapsed else ft.Text("ADMINISTRACIÓN", size=10, color=colors.TEXT_SECONDARY, weight=ft.FontWeight.BOLD),
                                    alignment=ft.alignment.center if self.collapsed else ft.alignment.center_left
                                ),
                                self._menu_item("Configuración", ft.Icons.SETTINGS, route="configuracion"),
                            ]),
                    ...
                    expand=True,
                    horizontal_alignment=ft.CrossAxisAlignment.CENTER if self.collapsed else ft.CrossAxisAlignment.START
                ),
                ...
                ft.Container(
                    ...
                    ) if self.collapsed else ft.TextButton(
                        "Cerrar Sesión",
                        icon=ft.Icons.LOGOUT,
                        icon_color=colors.ERROR,
    ...
    def _menu_item(self, titulo, icono, activado=False, route=None, on_click=None):
        ...
        if on_click:
            ...
            handler = lambda _: self.on_navigate(route_key)
        ...
        self._menu_items_map[route_key] = {
            ...
            # Text is optional now
             "text": content.controls[1] if not self.collapsed and isinstance(content, ft.Row) else None
        }
