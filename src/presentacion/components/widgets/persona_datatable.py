...
class PersonaDataTable(ft.Column):
    ...
    def _crear_fila(self, persona: PersonaConRoles) -> ft.DataRow:
        ...
        return ft.DataRow(
            cells=[
                # Nombre
                ft.DataCell(ft.Text(persona.nombre_completo, size=14, weight=ft.FontWeight.W_500)),
                # Documento
                ft.DataCell(ft.Text(persona.numero_documento, size=13)),
                # Celular
                ft.DataCell(ft.Text(persona.telefono_principal or "-", size=13)),
                # Correo
                ft.DataCell(
                    ft.Text(
                ...
                ft.DataCell(
                    ft.Row(
                        [RoleBadge(rol, small=True) for rol in persona.roles], spacing=5, wrap=True
                    )
                ),
                # Estado (Switch)
                ft.DataCell(
                    ft.Switch(
                        ...
                        on_change=lambda e, id_p=persona.persona.id_persona: self.on_toggle_estado(
                            id_p, e.control.value
                        ),
                    )
                ),
                # Acciones
                    ...
                    ft.Row(
                        [
                            ft.IconButton(
                                ...
                                on_click=lambda e, id_p=persona.persona.id_persona: self.on_edit(
                                    id_p
                                ),
                            )
                        ],
