...
class PaginationManager(ft.Container):
    ...
    def _build_ui(self):
        ...
        total_pages_text = ft.Text(f"de {self.total_pages}", size=14, color=colors.TEXT_SECONDARY)
        ...
        page_size_dropdown = ft.Dropdown(
            ...
            options=[
                ft.dropdown.Option(str(size), f"{size} items") for size in self.page_size_options
            ],
        ...
        # Texto informativo
        info_text = ft.Text(
            ref=self.info_text, value=self._get_info_text(), size=13, color=colors.TEXT_SECONDARY
        )
