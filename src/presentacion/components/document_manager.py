...
class DocumentManager(ft.Column):
    def __init__(self, entidad_tipo: str, entidad_id: str, page: ft.Page, **kwargs):
        ...
        self.file_picker = ft.FilePicker(
            on_result=self._on_file_result, on_upload=self._procesar_subida_web
        )
            ...
            ft.Row(
                [
                    ...
                    ft.Row(
                        [
                            ft.Text(
                                ref=ft.Ref[ft.Text](), visible=False
                            ),  # Placeholder for progress
                            ft.IconButton(
                                ft.Icons.UPLOAD_FILE,
                                tooltip="Adjuntar Archivo",
                                on_click=lambda _: self.abrir_dialogo_carga(),
                            ),
                        ]
                    ),
                ],
                                ...
                                ft.Row(
                                    [
                                        ft.IconButton(
                                            ...
                                            tooltip="Descargar",
                                            on_click=lambda _, id=doc.id: self.descargar_documento(
                                                id
                                            ),
                                        ),
                                        ft.IconButton(
                                            ...
                                            tooltip="Eliminar",
                                            on_click=lambda _, id=doc.id: self.eliminar_documento(
                                                id
                                            ),
                                        ),
                                    ]
    ...
    def mostrar_modal_descripcion(self):
        ...
        def confirmar_subida(e):
            ...
            if self.archivo_temporal.path:
                ...
                upload_file = ft.FilePickerUploadFile(
                    name=self.archivo_temporal.name, upload_url=upload_url
                )
        ...
        self.dlg_desc = ft.AlertDialog(
            ...
            content=ft.Column(
                [ft.Text(f"Archivo: {self.archivo_temporal.name}"), txt_descripcion_local],
                tight=True,
            ),
            actions=[
    ...
    def eliminar_documento(self, id_documento):
        ...
        self.dlg_eliminar = ft.AlertDialog(
            ...
            actions=[
                ...
                ft.TextButton(
                    "Eliminar", on_click=confirmar_eliminar, style=ft.ButtonStyle(color="red")
                ),
            ],
        )
