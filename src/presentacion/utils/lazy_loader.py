...
class LazyModule:
    ...
    def __init__(
        self, module_path: str, builder_function: str, preload_after_seconds: Optional[int] = None
    ):
        """
        ...
        try:
            ...
            if not hasattr(self._module, self.builder_function):
                raise AttributeError(
                    f"Módulo {self.module_path} no tiene función " f"'{self.builder_function}'"
                )

            self._builder = getattr(self._module, self.builder_function)
            self._load_time = time.time() - start_time

            logger.info(f"Módulo {self.module_path} cargado en {self._load_time:.3f}s")
    ...
    def preload_async(self) -> None:
        ...
        if self._module is None and not self._loading:
            ...
            threading.Thread(
                target=preload_task, daemon=True, name=f"Preload-{self.module_path}"
            ).start()
...
class LazyLoaderManager:
    ...
    def register(
        ...
        with self._lock:
            ...
            self.load_order = sorted(self.modules.items(), key=lambda x: x[1]["priority"])

            logger.debug(
                f"Registrado módulo lazy: {name} " f"(priority={priority}, path={module_path})"
            )
    ...
    def _schedule_preload(self, name: str, delay_seconds: int) -> None:
        ...
        threading.Thread(target=delayed_preload, daemon=True, name=f"DelayedPreload-{name}").start()
    ...
    def get_builder(self, name: str) -> Callable:
        ...
        with self._lock:
            if name not in self.modules:
                ...
                raise ValueError(f"Módulo no registrado: '{name}'. " f"Disponibles: {available}")
    ...
    def unregister(self, name: str) -> None:
        ...
        with self._lock:
            if name in self.modules:
                ...
                self.load_order = [(n, i) for n, i in self.load_order if n != name]
...
def register_lazy_module(
    ...
    """
    lazy_loader.register(name, module_path, builder_function, priority, preload_after)
