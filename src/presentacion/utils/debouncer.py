...
class Debouncer:
    ...
    def __init__(self, delay_ms: int, callback: Callable, name: Optional[str] = None):
        """
    ...
    def __call__(self, *args, **kwargs) -> None:
        ...
        with self._lock:
            ...
            if self._timer is not None:
                self._timer.cancel()
                logger.debug(
                    f"Debouncer '{self.name}': Timer cancelado " f"(call #{self._call_count})"
                )
            ...
            def execute_callback():
                ...
                logger.debug(
                    f"Debouncer '{self.name}': Ejecutando callback " f"(execute #{execute_num})"
                )

                try:
                    self.callback(*args, **kwargs)
                except Exception as e:
                    logger.error(f"Debouncer '{self.name}': Error en callback: {e}", exc_info=True)

            self._timer = threading.Timer(
                interval=self.delay_ms / 1000.0, function=execute_callback
            )
    ...
    def flush(self) -> None:
        ...
        with self._lock:
            if self._timer is not None:
                ...
                logger.debug(f"Debouncer '{self.name}': Flush - ejecutando inmediatamente")

                try:
                    self.callback()
                except Exception as e:
                    logger.error(f"Debouncer '{self.name}': Error en flush: {e}", exc_info=True)
    ...
    def get_stats(self) -> dict:
        ...
        with self._lock:
            ...
            if self._call_count > 0:
                reduction_rate = (self._call_count - self._execute_count) / self._call_count * 100
...
class ThrottledDebouncer(Debouncer):
    ...
    def __init__(
        self, delay_ms: int, callback: Callable, max_wait_ms: int, name: Optional[str] = None
    ):
        """
        ...
        with self._lock:
            ...
            if elapsed_ms >= self.max_wait_ms:
                ...
                try:
                    ...
                    logger.error(f"ThrottledDebouncer '{self.name}': Error: {e}", exc_info=True)
            else:
