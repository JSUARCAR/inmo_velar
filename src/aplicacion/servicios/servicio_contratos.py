...
class ServicioContratos:
    ...
    def crear_mandato(self, datos: Dict, usuario_sistema: str) -> ContratoMandato:
        ...
        if mandato_existente:
            raise ValueError(f"La propiedad ya tiene un contrato de mandato activo (ID: {mandato_existente.id_contrato_m})")
    ...
    def crear_arrendamiento(self, datos: Dict, usuario_sistema: str) -> ContratoArrendamiento:
        ...
        if arriendo_existente:
            raise ValueError(f"La propiedad ya tiene un contrato de arrendamiento activo (ID: {arriendo_existente.id_contrato_a})")
    ...
    def renovar_arrendamiento(self, id_contrato: int, usuario_sistema: str) -> ContratoArrendamiento:
        ...
        day_nuevo = fecha_fin_actual.day
        ...
        try:
            ...
            if mes_nuevo == 2:
                nueva_fecha_fin_dt = fecha_fin_actual.replace(year=anio_nuevo, month=mes_nuevo, day=28)
            else:
                 nueva_fecha_fin_dt = fecha_fin_actual.replace(year=anio_nuevo, month=mes_nuevo, day=30)

        nueva_fecha_fin_str = nueva_fecha_fin_dt.strftime("%Y-%m-%d")
        fecha_inicio_renovacion = (fecha_fin_actual).strftime("%Y-%m-%d") # La renovación "inicia" donde termina el anterior para efectos de historial
            ...
            return [
                {
                    ...
                    "propiedad": row['DIRECCION_PROPIEDAD'],
                    "arrendatario": row['ARRENDATARIO'],
                    "dias_restantes": (datetime.strptime(row['FECHA_FIN_CONTRATO_A'], "%Y-%m-%d") - datetime.now()).days
                }
                for row in cursor.fetchall()
            ]
    ...
    def exportar_contratos_csv(
        ...
        def get_val(row: dict, keys: list) -> Any:
            for k in keys:
                ...
                if k.lower() in row: return row[k.lower()]
        ...
        if filtro_tipo in ["Todos", "Mandato"]:
            ...
            for m in resultado_mandatos:
                items.append({
                    ...
                    "persona": get_val(m, ["PROPIETARIO", "nombre_propietario", "nombre_completo"]),
        ...
        # Header
        writer.writerow([
            "ID", "TIPO", "ESTADO", "PROPIEDAD", 
            "PERSONA (Propietario/Inquilino)", "DOCUMENTO", 
            "CANON", "FECHA INICIO", "FECHA FIN"
        ])
    ...
    def _listar_todos_mandatos(self, estado, busqueda):
        ...
        with self.db.obtener_conexion() as conn:
            ...
            if estado and estado != "Todos":
                if estado == "Activo": conditions.append("cm.ESTADO_CONTRATO_M = 'Activo'")
                elif estado == "Cancelado": conditions.append("cm.ESTADO_CONTRATO_M != 'Activo'")
                else: 
                     conditions.append(f"cm.ESTADO_CONTRATO_M = {placeholder}")
                     params.append(estado)
            
            if busqueda:
                conditions.append(f"(p.DIRECCION_PROPIEDAD LIKE {placeholder} OR per.NOMBRE_COMPLETO LIKE {placeholder})")
    ...
    def _listar_todos_arrendamientos(self, estado, busqueda):
        ...
        with self.db.obtener_conexion() as conn:
            ...
            if estado and estado != "Todos":
                if estado == "Activo": conditions.append("ca.ESTADO_CONTRATO_A = 'Activo'")
                elif estado == "Cancelado": conditions.append("ca.ESTADO_CONTRATO_A != 'Activo'")
                else: 
                     conditions.append(f"ca.ESTADO_CONTRATO_A = {placeholder}")
                     params.append(estado)
    ...
    def verificar_vencimientos(self, usuario_sistema: str = "sistema"):
        ...
        with self.repo_mandato.db.obtener_conexion() as conn:
            ...
            for a in arriendos_ipc:
                ...
                fecha_inicio = datetime.strptime(a[1], "%Y-%m-%d")
                fecha_target = datetime.now() # Aprox, para el mensaje
                # Aniversario numero? 
    ...
    def aplicar_incremento_ipc(
        ...
        try:
            ...
            try:
                ...
                estado = row[2]
                if estado != 'Activo':
                    return {'success': False, 'message': "Solo se puede aplicar IPC a contratos activos"}
                ...
                if ultimo_incremento:
                    ...
                    try:
                        ultima_fecha = ultimo_incremento if isinstance(ultimo_incremento, (datetime, list)) else datetime.strptime(str(ultimo_incremento), "%Y-%m-%d")
                        if isinstance(ultima_fecha, str):
                             ultima_fecha = datetime.strptime(ultima_fecha, "%Y-%m-%d")
                             
                        nueva_fecha = datetime.strptime(fecha_aplicacion, "%Y-%m-%d")
                        
                        if (ultima_fecha.year == nueva_fecha.year and 
                            ultima_fecha.month == nueva_fecha.month):
                            return {'success': False, 'message': f"Ya se aplicó incremento en {ultima_fecha.strftime('%Y-%m')}"}
                    except Exception as e:
                         pass 
                ...
                try:
                    ...
                    """, (id_contrato, fecha_aplicacion, porcentaje_ipc,
                          canon_anterior, canon_nuevo, observaciones, usuario))
                except Exception:
                   pass
