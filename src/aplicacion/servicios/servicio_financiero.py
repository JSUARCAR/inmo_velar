...
class ServicioFinanciero:
    ...
    def registrar_recaudo(self, datos: Dict[str, Any], conceptos_data: List[Dict[str, Any]], 
                         usuario_sistema: str) -> Recaudo:
        """
    ...
    def calcular_mora(self, id_contrato_a: int, fecha_limite: str, fecha_pago: str, 
                     valor_canon: int) -> int:
        """
    ...
    def aplicar_pago_anticipado(self, id_contrato_a: int, meses_adelantados: int,
                                valor_canon_mensual: int, fecha_pago: str,
                                metodo_pago: str, referencia_bancaria: Optional[str],
                                usuario_sistema: str) -> Recaudo:
        """
    ...
    def generar_liquidacion_mensual(self, id_contrato_m: int, periodo: str, 
                                    datos_adicionales: Dict[str, Any],
                                    usuario_sistema: str) -> Liquidacion:
        """
        ...
        comision_porcentaje = datos_adicionales.get('comision_porcentaje', 
    ...
    def marcar_liquidacion_pagada(self, id_liquidacion: int, fecha_pago: str,
                                  metodo_pago: str, referencia_pago: str,
                                  usuario_sistema: str) -> None:
        """
    ...
    def cancelar_liquidacion(self, id_liquidacion: int, motivo: str, 
                            usuario_sistema: str) -> None:
        """
    ...
    def cancelar_liquidaciones_masivas(self, ids_liquidaciones: List[int], 
                                      motivo: str, usuario_sistema: str) -> Dict[str, List[int]]:
        """
        ...
        """
        return self.repo_liquidacion.cancelar_masivamente(ids_liquidaciones, motivo, usuario_sistema)
    
    def reversar_liquidaciones_masivas(self, ids_liquidaciones: List[int], 
                                       usuario_sistema: str) -> Dict[str, List[int]]:
        """
    ...
    def cancelar_liquidaciones_por_propietario(self, id_propietario: int, periodo: str,
                                               motivo: str, usuario_sistema: str) -> int:
        """
    ...
    def reversar_liquidaciones_por_propietario(self, id_propietario: int, periodo: str,
                                               usuario_sistema: str) -> int:
        """
            ...
            items = [
                {
                    ...
                    'contrato': row['DIRECCION_PROPIEDAD'] # Mapeamos dirección como 'contrato' para compatibilidad UI
                }
    ...
    def aprobar_recaudo(self, id_recaudo: int, usuario_sistema: str) -> None:
        ...
        if recaudo.estado_recaudo != 'Pendiente':
            raise ValueError(f"Solo se pueden aprobar recaudos en estado Pendiente (Estado actual: {recaudo.estado_recaudo})")
    ...
    def reversar_recaudo(self, id_recaudo: int, usuario_sistema: str) -> None:
        ...
        if recaudo.estado_recaudo != 'Pendiente':
            raise ValueError(f"Solo se pueden reversar recaudos en estado Pendiente (Estado actual: {recaudo.estado_recaudo})")
    ...
    def obtener_datos_consolidados_para_pdf(self, propietario_id: int, periodo: str) -> Optional[Dict[str, Any]]:
        """
        ...
        with self.db.obtener_conexion() as conn:
            ...
            for row in rows:
                ...
                egresos_prop = (
                    row['COMISION_MONTO'] + row['IVA_COMISION'] + 
                    row['IMPUESTO_4X1000'] + row['GASTOS_ADMINISTRACION'] + 
                    row['GASTOS_SERVICIOS'] + row['GASTOS_REPARACIONES'] + 
                    row['OTROS_EGRESOS']
                )
            ...
            total_egresos = (
                total_comision + total_iva + total_4x1000 + 
                total_gastos_admin + total_gastos_serv + 
                total_gastos_rep + total_otros_egr
            )
    ...
    def actualizar_liquidacion(self, id_liquidacion: int, datos_actualizados: Dict[str, Any], 
                               usuario_sistema: str) -> Liquidacion:
        """
        ...
        if liquidacion.estado_liquidacion != 'En Proceso':
            raise ValueError(f"Solo se pueden editar liquidaciones en estado 'En Proceso'. Estado actual: {liquidacion.estado_liquidacion}")
        ...
        if 'comision_porcentaje' in datos_actualizados:
            ...
            liquidacion.comision_monto = int((liquidacion.canon_bruto * liquidacion.comision_porcentaje) / 10000)
    ...
    def generar_liquidacion_propietario(self, id_propietario: int, periodo: str,
                                        datos_adicionales_por_contrato: Optional[Dict[int, Dict[str, Any]]] = None,
                                        usuario_sistema: str = "sistema") -> LiquidacionPropietario:
        """
        ...
        if not contratos_activos:
            raise ValueError(f"El propietario {id_propietario} no tiene contratos de mandato activos")
        
        # Verificar que no existan ya liquidaciones para este período
        liquidaciones_existentes = self.repo_liquidacion.listar_por_propietario_y_periodo(id_propietario, periodo)
        if liquidaciones_existentes:
            raise ValueError(f"Ya existen liquidaciones para el propietario {id_propietario} en el período {periodo}")
        ...
        for contrato in contratos_activos:
            ...
            datos_contrato = datos_adicionales_por_contrato.get(contrato.id_contrato_m, {}) if datos_adicionales_por_contrato else {}
            ...
            try:
                ...
                raise ValueError(f"Error generando liquidación para contrato {contrato.id_contrato_m}: {str(e)}")
    ...
    def generar_liquidaciones_masivas(
        ...
        for id_prop in ids_propietarios:
            try:
                liq_prop = self.generar_liquidacion_propietario(id_prop, periodo, usuario_sistema=usuario_sistema)
    ...
    def obtener_detalle_liquidacion_propietario_ui(self, id_propietario: int, periodo: str) -> Optional[Dict[str, Any]]:
        """
        ...
        return {
            **liq_consolidada.obtener_resumen_dict(),
            'contratos': detalles_contratos
        }
    
    def aprobar_liquidacion_propietario(self, id_propietario: int, periodo: str, usuario_sistema: str) -> int:
        """
    ...
    def marcar_liquidacion_propietario_pagada(self, id_propietario: int, periodo: str,
                                              fecha_pago: str, metodo_pago: str,
                                              referencia_pago: str, usuario_sistema: str) -> int:
        """
