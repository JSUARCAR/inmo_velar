...
class ServicioDocumental:
    ...
    def subir_documento(
        ...
        ultima_version = self.repositorio.obtener_ultima_version(
...
class ServicioDocumentalElite(ServicioDocumental):
    ...
    def validar_documento_modulo(
        ...
        if not reglas_tipo:
            return {
                "valido": False,
                "mensaje": f"Tipo de documento {tipo_documento} no válido para {entidad_tipo}",
            }
        ...
        if ext not in reglas_tipo["tipos"]:
            return {
                "valido": False,
                "mensaje": f"Tipo de archivo no permitido. Esperado: {', '.join(reglas_tipo['tipos'])}",
            }
        ...
        if len(archivo_bytes) > reglas_tipo["max_size"]:
            ...
            return {
                "valido": False,
                "mensaje": f"El archivo excede el tamaño máximo de {limit_mb:.1f}MB",
            }
