...
def truncar_observaciones(
    ...
    ids_incidentes = [
        linea for linea in lineas if linea.strip().startswith(PREFIJO_INCIDENTE)
    ]
    otros = [
        linea for linea in lineas if not linea.strip().startswith(PREFIJO_INCIDENTE)
    ]
...
class ServicioIncidenteLiquidacion:
    ...
    def asociar_incidente(
        ...
        try:
            with db_manager.transaccion():
                ...
                cuota = next(
                    (c for c in cuotas if c.numero_cuota == numero_cuota), None
                )
                ...
                relaciones_existentes = self.repositorio_relacion.obtener_por_incidente(
                    id_incidente
                )
                duplicado = next(
                    (
                ...
                if duplicado:
                    ...
                    total_descuentos = (
                        self.repositorio_relacion.calcular_total_descuentos(
                            id_liquidacion
                        )
                    )
                    return {
    ...
    def desasociar_incidente(
        ...
        try:
            with db_manager.transaccion():
                ...
                plan = self.repositorio_plan.obtener_por_incidente(
                ...
                if liquidacion:
                    ...
                    self.repositorio_liquidacion.actualizar(
                        liquidacion, desasociado_por
                    )
                ...
                logger.info(f"Relación {id_relacion} eliminada por {desasociado_por}")
    ...
    def obtener_incidentes_por_liquidacion(self, id_liquidacion: int) -> Dict[str, Any]:
        """
    ...
    def obtener_liquidaciones_por_incidente(self, id_incidente: int) -> Dict[str, Any]:
        """
