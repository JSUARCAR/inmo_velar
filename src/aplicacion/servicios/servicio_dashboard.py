...
class ServicioDashboard:
    ...
    def obtener_flujo_caja_mes(
        ...
        with self.db.obtener_conexion() as conn:
            ...
            recaudado = (
                recaudado_res["TOTAL_RECAUDO"]
                if recaudado_res and recaudado_res["TOTAL_RECAUDO"]
                else 0
            )
            ...
            esperado = (
                esperado_res["TOTAL_ESPERADO"]
                if esperado_res and esperado_res["TOTAL_ESPERADO"]
                else 0
            )
    ...
    def obtener_tunel_vencimientos(self) -> List[Dict]:
        ...
        with self.db.obtener_conexion() as conn:
            ...
            cursor.execute(query)
            rows = cursor.fetchall()
            return [{"mes": row["MES"], "valor_riesgo": float(row["VALOR_RIESGO"])} for row in rows]
    ...
    def obtener_evolucion_recaudo(
        self, meses: int = 6, mes_fin: int = None, anio_fin: int = None
    ) -> Dict:
        """
        ...
        with self.db.obtener_conexion() as conn:
            ...
            return {"etiquetas": fechas, "valores": valores}
    ...
    def obtener_desempeno_asesores(self) -> Dict:
        ...
        with self.db.obtener_conexion() as conn:
            ...
            top_comisiones = [
                {"nombre": row["NOMBRE_COMPLETO"], "comisiones": row["COMISIONES_MES"]}
                for row in cursor.fetchall()
            ]

            return {"top_contratos": top_asesores, "top_comisiones": top_comisiones}
