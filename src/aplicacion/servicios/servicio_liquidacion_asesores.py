...
class ServicioLiquidacionAsesores:
    ...
    def generar_liquidacion_multi_contrato(
        ...
        """
        print(
            f"[SERVICE] Iniciando generar_liquidacion_multi_contrato para Asesor {id_asesor}, Periodo {periodo}"
        )

        # Validar que no exista liquidación duplicada (asesor + período)
        try:
            existente = self.repo_liquidacion.obtener_por_asesor_periodo(
                id_asesor, periodo
            )
            if existente:
                print(
                    f"[SERVICE] Error: Ya existe liquidación para Asesor {id_asesor} en {periodo}"
                )
                raise ValueError(
        ...
        print(
            f"[SERVICE] Canon total calculado: {canon_total} sobre {len(contratos_lista)} contratos"
        )

        # Calcular comisión bruta sobre el total
        comision_bruta = LiquidacionAsesor.calcular_comision_bruta(
            canon_total, porcentaje_comision
        )
        print(
            f"[SERVICE] Comisión bruta: {comision_bruta} (Porcentaje: {porcentaje_comision})"
        )
        ...
        try:
            ...
            print(
                f"[SERVICE] Liquidación guardada con ID: {liquidacion_creada.id_liquidacion_asesor}"
            )
        except Exception as e:
    ...
    def agregar_descuento(
        ...
        if not liquidacion.puede_editarse:
            raise ValueError(
                "Solo se pueden agregar descuentos a liquidaciones pendientes"
            )
    ...
    def _recalcular_valor_neto(self, id_liquidacion: int, usuario: str) -> None:
        ...
        if self.repo_bonificacion:
            total_bonificaciones = self.repo_bonificacion.calcular_total_bonificaciones(
                id_liquidacion
            )
        else:
    ...
    def eliminar_bonificacion(self, id_bonificacion: int, usuario: str) -> bool:
        ...
        if self.repo_bonificacion.eliminar(id_bonificacion):
            ...
            if liquidacion:
                ...
                liquidacion.valor_neto = (
                    liquidacion.comision_bruta + liquidacion.total_bonificaciones
                ) - total_descuentos
    ...
    def remover_bonificacion_consolidada(
        self, id_liquidacion: int, usuario: str
    ) -> bool:
        """
    ...
    def programar_pago(
        ...
        pago = PagoAsesor(
            ...
            referencia_pago=datos_adicionales.get("referencia")
    ...
    def listar_liquidaciones(
        ...
        liquidaciones = self.repo_liquidacion.listar_con_filtros(
            id_asesor=id_asesor, periodo=periodo, estado=estado
        )
    ...
    def listar_liq_asesores_paginado(
        ...
        with db_manager.obtener_conexion() as conn:
            ...
            for row in cursor.fetchall():
                ...
                pct = (
                    float(row["porcentaje_comision"]) / 100.0
                    if row["porcentaje_comision"]
                    else 0.0
                )
                ...
                items.append(
                    {
                        ...
                        "puede_anularse": estado
                        not in ["Anulada", "Pagada"],  # Según lógica de negocio típica
                    }
                )
    ...
    def obtener_detalle_completo(self, id_liquidacion: int) -> Dict[str, Any]:
        ...
        if self.repo_bonificacion:
            ...
            try:
                with self.repo_liquidacion.db_manager.obtener_conexion() as conn:
                    ...
                    bonificaciones = [
                        {
                            "id_bonificacion_asesor": row.get("ID_BONIFICACION_ASESOR")
                            ...
                            or row.get("tipo_bonificacion"),
                            "descripcion_bonificacion": row.get(
                                "DESCRIPCION_BONIFICACION"
                            )
                            or row.get("descripcion_bonificacion"),
                            "valor_bonificacion": row.get("VALOR_BONIFICACION")
                            or row.get("valor_bonificacion"),
                            "fecha_registro": row.get("FECHA_REGISTRO")
                            or row.get("fecha_registro"),
                        }
        ...
        if liquidacion.id_contrato_a and self.repo_contrato_arrendamiento:
            try:
                contrato = self.repo_contrato_arrendamiento.obtener_por_id(
    ...
    def obtener_resumen_por_asesor(self, id_asesor: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "pendientes_aprobacion": sum(
                1 for liq in liquidaciones if liq.esta_pendiente
            ),
    ...
    def _liquidacion_to_dict(self, liq: LiquidacionAsesor) -> Dict[str, Any]:
        ...
        return {
            ...
            "nombre_asesor": liq.nombre_asesor
