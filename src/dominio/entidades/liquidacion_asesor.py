...
class LiquidacionAsesor:
    ...
    def __post_init__(self):
        ...
        if self.porcentaje_comision < 0 or self.porcentaje_comision > 10000:
            raise ValueError(
                "El porcentaje de comisión debe estar entre 0 y 10000 (0.00% - 100.00%)"
            )
        ...
        if self.estado_liquidacion not in self.ESTADOS:
            raise ValueError(
                f"Estado inválido: {self.estado_liquidacion}. "
        ...
        if self.periodo_liquidacion and len(self.periodo_liquidacion) > 0:
            ...
            if not re.match(r"^\d{4}-\d{2}$", self.periodo_liquidacion):
                raise ValueError(
                    f"Formato de período inválido: {self.periodo_liquidacion}. "
                    "Use formato YYYY-MM"
                )
    ...
    def recalcular_valor_neto(
        ...
        self.valor_neto_asesor = self.calcular_valor_neto(
            self.total_descuentos, self.total_bonificaciones
        )
    ...
    def aprobar(self, usuario_aprobador: str) -> None:
        ...
        if not self.puede_aprobarse:
            raise ValueError(
                f"No se puede aprobar una liquidación en estado {self.estado_liquidacion}"
            )
