...
class LiquidacionPropietario:
    ...
    def __post_init__(self):
        ...
        if self.estado_consolidado not in [
            "En Proceso",
            "Aprobada",
            "Pagada",
            "Mixto",
    ...
    def consolidar(self):
        ...
        estados = {"En Proceso": 0, "Aprobada": 0, "Pagada": 0, "Cancelada": 0}
        ...
        if estados["En Proceso"] > 0:
            ...
            primera_pagada = next(
                (liq for liq in self.liquidaciones_contratos if liq.esta_pagada), None
            )
            if primera_pagada:
