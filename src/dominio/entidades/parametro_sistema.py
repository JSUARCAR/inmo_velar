...
class ParametroSistema:
    ...
    def __post_init__(self):
        ...
        if self.tipo_dato and self.tipo_dato not in ('INTEGER', 'TEXT', 'DECIMAL', 'BOOLEAN'):
            raise ValueError(f"Tipo de dato inválido: {self.tipo_dato}. "
    ...
    def valor_como_decimal(self) -> Decimal:
        ...
        if self.tipo_dato not in ('DECIMAL', 'INTEGER'):
            raise TypeError(f"El parámetro '{self.nombre_parametro}' no es de tipo DECIMAL o INTEGER")
        try:
    ...
    def valor_como_porcentaje(self) -> Decimal:
        ...
        """
        if self.tipo_dato != 'INTEGER':
            raise TypeError(f"El parámetro '{self.nombre_parametro}' debe ser INTEGER para conversión a porcentaje")
    ...
    def actualizar_valor(self, nuevo_valor: str, usuario: str) -> None:
        ...
        if self.tipo_dato == 'INTEGER':
            try:
                ...
                raise ValueError(f"El valor debe ser un número entero válido")
        elif self.tipo_dato == 'DECIMAL':
            try:
                Decimal(nuevo_valor)
            except Exception:
                raise ValueError(f"El valor debe ser un número decimal válido")
        elif self.tipo_dato == 'BOOLEAN':
            if nuevo_valor not in ('0', '1', 'true', 'false', 'True', 'False'):
                raise ValueError(f"El valor debe ser un booleano válido: 0, 1, true, false")
