...
class PDFState(rx.State):
    ...
    def generar_contrato_arrendamiento_elite(
        ...
        """
        logger.info(f"="*80)
        logger.info(f"🖱️  FRONTEND: Button clicked! Event handler triggered")
        ...
        logger.info(f"📍 Call stack entry point: generar_contrato_arrendamiento_elite")
        logger.info(f"🚀 INICIANDO GENERACIÓN DE CONTRATO ÉLITE")
        ...
        try:
            ...
            pdf_path = self.pdf_service.generar_contrato_elite(
                datos,
                usar_borrador=es_borrador
            )
            ...
            logger.info(f"✅ CONTRATO GENERADO EXITOSAMENTE")
            ...
            logger.error(f"❌ ERROR EN GENERACIÓN DE CONTRATO")
    ...
    def generar_contrato_mandato_elite(
        ...
        """
        logger.info(f"="*80)
        logger.info(f"🖱️  FRONTEND: Button clicked - Mandato Contract!")
        logger.info(f"🔢 Parameters: contrato_id={contrato_id}, es_borrador={es_borrador}")
        logger.info(f"⏱️  Timestamp: {datetime.now().isoformat()}")
        logger.info(f"🚀 INICIANDO GENERACIÓN DE CONTRATO MANDATO ÉLITE")
        ...
        try:
            ...
            logger.info(f"✅ CONTRATO MANDATO GENERADO EXITOSAMENTE")
            logger.info(f"Path: {pdf_path}")
            ...
            logger.error(f"❌ ERROR EN GENERACIÓN DE CONTRATO MANDATO")
            logger.error(f"Tipo: {type(e).__name__}")
            logger.error(f"Mensaje: {str(e)}")
            logger.error(f"Traceback:\n{traceback.format_exc()}")
    ...
    def generar_certificado_paz_y_salvo(
        ...
        try:
            ...
            logger.error(f"❌ ERROR EN GENERACIÓN DE CERTIFICADO PAZ Y SALVO")
            logger.error(f"Tipo: {type(e).__name__}")
            logger.error(f"Mensaje: {str(e)}")
            logger.error(f"Traceback:\n{traceback.format_exc()}")
    ...
    def generar_liquidacion_pdf(self, id_liquidacion: int):
        ...
        """
        logger.info(f"="*80)
        logger.info(f"💰 INICIANDO GENERACIÓN DE PDF LIQUIDACIÓN")
        ...
        try:
            ...
            logger.info(f"✅ LIQUIDACIÓN PDF GENERADO EXITOSAMENTE")
            logger.info(f"Path: {pdf_path}")
            
            yield rx.toast.success(f"PDF de liquidación generado exitosamente")
            ...
            logger.error(f"❌ ERROR EN GENERACIÓN DE PDF LIQUIDACIÓN")
            logger.error(f"Tipo: {type(e).__name__}")
            logger.error(f"Mensaje: {str(e)}")
            logger.error(f"Traceback:\n{traceback.format_exc()}")
    ...
    def generar_estado_cuenta_elite(self, propietario_id: int = None, periodo: str = None, liquidacion_id: int = None):
        """
        ...
        """
        logger.info(f"="*80)
        logger.info(f"💰 INICIANDO GENERACIÓN DE ESTADO DE CUENTA ÉLITE")
        ...
        logger.info(f"Modo: Legacy por propietario/período")
        logger.info(f"Propietario ID: {propietario_id}")
        logger.info(f"Período: {periodo}")
        logger.info(f"Timestamp: {datetime.now()}")
        
        logger.info(f"🖱️  FRONTEND: Estado de Cuenta button clicked!")
        logger.info(f"🔄 Estado generando: False → True")
        self.generating = True
        ...
        try:
            ...
            logger.info(f"✅ ESTADO DE CUENTA GENERADO EXITOSAMENTE")
            logger.info(f"Path: {pdf_path}")
            ...
            logger.error(f"❌ ERROR EN GENERACIÓN DE ESTADO DE CUENTA")
            logger.error(f"Tipo: {type(e).__name__}")
            logger.error(f"Mensaje: {str(e)}")
            logger.error(f"Traceback:\n{traceback.format_exc()}")
    ...
    def _get_datos_estado_cuenta(
        ...
        if not datos:
            raise ValueError(f"No se encontraron liquidaciones para propietario {propietario_id} en período {periodo}")
    ...
    def _transform_consolidated_to_pdf_format(self, datos: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for idx, prop in enumerate(propiedades, 1):
            ...
            egresos_prop = (
                prop['comision_monto'] + prop['iva_comision'] + 
                prop['impuesto_4x1000'] + prop['gastos_admin'] + 
                prop['gastos_serv'] + prop['gastos_rep'] + 
                prop['otros_egr']
            )
            if egresos_prop > 0:
                movimientos.append({
        ...
        resumen = {
            ...
            'otros_descuentos': (
                datos['impuesto_4x1000'] + datos['gastos_admin'] + 
                datos['gastos_serv'] + datos['gastos_rep'] + datos['otros_egr']
            ),
        ...
        return {
            'estado_id': abs(hash(f"{datos['propietario']}-{datos['periodo']}")) % 1000000,  # Generate pseudo ID
    ...
    def generar_liquidacion_asesor_pdf(self, id_liquidacion_asesor: int):
        ...
        """
        logger.info(f"="*80)
        logger.info(f"💼 INICIANDO GENERACIÓN DE PDF LIQUIDACIÓN ASESOR")
        ...
        try:
            ...
            logger.info(f"✅ PDF LIQUIDACIÓN ASESOR GENERADO EXITOSAMENTE")
            logger.info(f"Path: {pdf_path}")
            ...
            logger.error(f"❌ ERROR EN GENERACIÓN DE PDF LIQUIDACIÓN ASESOR")
            logger.error(f"Tipo: {type(e).__name__}")
            logger.error(f"Mensaje: {str(e)}")
            logger.error(f"Traceback:\n{traceback.format_exc()}")
    ...
    def generar_recibo_pago_pdf(self, id_recaudo: int):
        ...
        """
        logger.info(f"="*80)
        logger.info(f"🧾 INICIANDO GENERACIÓN DE PDF RECIBO DE PAGO")
        ...
        try:
            ...
            logger.debug(f"✅ Datos obtenidos: Recaudo {row['ID_RECAUDO']}, Valor ${row['VALOR_TOTAL']:,}")
            ...
            if conceptos_rows:
                ...
                periodo = (conceptos_rows[0].get('periodo') or 
                          conceptos_rows[0].get('PERIODO') or 
                          periodo)
            ...
            logger.info(f"✅ PDF RECIBO DE PAGO GENERADO EXITOSAMENTE")
            logger.info(f"Path: {pdf_path}")
            ...
            logger.error(f"❌ ERROR EN GENERACIÓN DE PDF RECIBO DE PAGO")
            logger.error(f"Tipo: {type(e).__name__}")
            logger.error(f"Mensaje: {str(e)}")
            logger.error(f"Traceback:\n{traceback.format_exc()}")
