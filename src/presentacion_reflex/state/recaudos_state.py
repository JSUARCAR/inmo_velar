    ...
    async def load_recaudos(self):
        ...
        try:
            ...
            with db_manager.obtener_conexion() as conn:
                ...
                total = (total_row.get('total') or total_row.get('TOTAL') or total_row.get('count') or 0) if total_row else 0
    ...
    async def save_recaudo(self, form_data: Dict):
        ...
        try:
            ...
            if isinstance(id_contrato, str) and not id_contrato.isdigit():
                ...
                contrato_opt = next((c for c in self.contratos_options if c["texto"] == id_contrato), None)
                if contrato_opt:
