...
class UsuariosState(rx.State):
            ...
            for u in users_list:
                ...
                if self.filter_status != "Todos":
                    ...
                    if self.filter_status == "Inactivo" and is_active:
                        pass_filter = False
                if self.search_text and self.search_text.lower() not in item["nombre_usuario"].lower():
                    pass_filter = False
        ...
        try:
            ...
            if self.is_editing:
                ...
                if datos["contrasena"]:
                    ...
                    servicio.restablecer_contrasena(datos["id_usuario"], datos["contrasena"], admin_user)
        ...
        try:
            ...
            for categoria, permisos_list in permisos_agrupados.items():
                ...
                for mod_name, mod_perms in modulos_map.items():
                    modules_list.append(PermissionModule(
                        name=mod_name,
                        permissions=mod_perms
                    ))
                
                # Agregar a la jerarquía
                hierarchy.append(PermissionCategory(
                    category=categoria,
    ...
    def toggle_all_module_permissions(self, modulo: str):
        ...
        module_permission_ids = [
            p["id_permiso"] for p in self.all_permissions 
            if p["modulo"] == modulo
        ]
        ...
        if all_selected:
            # Deseleccionar todos
            self.role_permissions_ids = [
                pid for pid in self.role_permissions_ids 
