...
class IncidentesState(DocumentosStateMixin):
    ...
    estado_options: List[str] = ["Todos", "Reportado", "En Revision", "Cotizado", "Aprobado", "En Reparacion", "Finalizado", "Cancelado"]
    ...
    kanban_columns: Dict[str, List[str]] = {
        "Reportado": ["Reportado", "En Revision"], # Agrupamos En Revision aquí o podría tener su propia col
        "Cotizado": ["Cotizado"],
        "Aprobado": ["Aprobado"],
        "En Reparacion": ["En Reparacion"],
        "Finalizado": ["Finalizado", "Cancelado"] # Agrupamos Cancelado aquí por ahora o lo ocultamos
    }
    ...
    responsable_pago_options: List[str] = ["Inquilino", "Propietario", "Inmobiliaria", "Aseguradora"]
        ...
        try:
            ...
            for inc in resultado_objs:
                ...
                item = {
                    ...
                    "fecha": inc.fecha_incidente.isoformat() if hasattr(inc.fecha_incidente, 'isoformat') else str(inc.fecha_incidente),
    ...
    async def load_proveedores(self):
        ...
        try:
            ...
            options = [
                {"id": str(r["ID_PROVEEDOR"]), "texto": f"{r['NOMBRE_COMPLETO']} ({r['ESPECIALIDAD']})"}
                for r in rows
            ]
        ...
        try:
            ...
            if detail_cots := detalle.get("cotizaciones"):
                ...
                for cot in detail_cots:
                    ...
                    if cot.id_proveedor:
                        ...
                        prov_opt = next((p for p in self.proveedores_options if p["id"] == str(cot.id_proveedor)), None)
                        if prov_opt:
            ...
            async with self:
                ...
                if prop := detalle.get("propiedad"):
                    current_inc["direccion_propiedad"] = getattr(prop, "direccion_propiedad", str(prop))
                ...
                if pid := inc_obj.id_proveedor_asignado: # Usar del objeto real, mas seguro
                    ...
                    prov_opt = next((p for p in self.proveedores_options if p["id"] == str(pid)), None)
                    if prov_opt:
    ...
    async def aprobar_cotizacion_event(self, id_incidente: int, id_cotizacion: int):
        ...
        try:
            ...
            servicio.aprobar_cotizacion(id_incidente, id_cotizacion, usuario, responsable_pago="Propietario")
    ...
    async def finalizar_carga_cotizaciones(self, id_incidente: int):
        ...
        try:
            ...
            yield rx.toast.success("Cotizaciones finalizadas. Incidente pasa a estado Cotizado para aprobación.")
            yield IncidentesState.load_incidentes()
