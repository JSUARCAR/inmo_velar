...
### Objetivo
Gestionar y supervisar de forma integral la información y operaciones relacionadas con **Liquidación de Asesores** dentro del ecosistema de Inmobiliaria Velar.

### Alcance
Este módulo abarca la creación, consulta, modificación y seguimiento de todos los registros de Liquidación de Asesores, permitiendo un control centralizado.
...
> En el contexto de **Liquidación de Asesores**, un registro representa una entidad única que atraviesa un ciclo de vida definido por reglas de negocio estrictas.
...
- **Ruta de acceso**: `Menú Principal → Liquidación de Asesores`
...
La pantalla principal de **Liquidación de Asesores** se compone de los siguientes elementos:
...
### 5.1. Crear un registro de Liquidación de Asesores
...
```mermaid
flowchart TD
    A[Inicio] --> B(Ingresar a Liquidación de Asesores)
...
> Todo cambio realizado en el módulo de **Liquidación de Asesores** genera automáticamente un registro en el log de auditoría. Los registros no se eliminan físicamente (Soft-Delete) para preservar la integridad referencial de los datos.
