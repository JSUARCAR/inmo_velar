...
### Objetivo
Gestionar y supervisar de forma integral la información y operaciones relacionadas con **Alertas Tempranas** dentro del ecosistema de Inmobiliaria Velar.

### Alcance
Este módulo abarca la creación, consulta, modificación y seguimiento de todos los registros de Alertas Tempranas, permitiendo un control centralizado.
...
> En el contexto de **Alertas Tempranas**, un registro representa una entidad única que atraviesa un ciclo de vida definido por reglas de negocio estrictas.
...
- **Ruta de acceso**: `Menú Principal → Alertas Tempranas`
...
La pantalla principal de **Alertas Tempranas** se compone de los siguientes elementos:
...
### 5.1. Crear un registro de Alertas Tempranas
...
```mermaid
flowchart TD
    A[Inicio] --> B(Ingresar a Alertas Tempranas)
...
> Todo cambio realizado en el módulo de **Alertas Tempranas** genera automáticamente un registro en el log de auditoría. Los registros no se eliminan físicamente (Soft-Delete) para preservar la integridad referencial de los datos.
