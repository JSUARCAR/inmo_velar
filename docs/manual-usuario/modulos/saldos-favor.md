...
### Objetivo
Gestionar y supervisar de forma integral la información y operaciones relacionadas con **Saldos a Favor** dentro del ecosistema de Inmobiliaria Velar.

### Alcance
Este módulo abarca la creación, consulta, modificación y seguimiento de todos los registros de Saldos a Favor, permitiendo un control centralizado.
...
> En el contexto de **Saldos a Favor**, un registro representa una entidad única que atraviesa un ciclo de vida definido por reglas de negocio estrictas.
...
- **Ruta de acceso**: `Menú Principal → Saldos a Favor`
...
La pantalla principal de **Saldos a Favor** se compone de los siguientes elementos:
...
### 5.1. Crear un registro de Saldos a Favor
...
```mermaid
flowchart TD
    A[Inicio] --> B(Ingresar a Saldos a Favor)
...
> Todo cambio realizado en el módulo de **Saldos a Favor** genera automáticamente un registro en el log de auditoría. Los registros no se eliminan físicamente (Soft-Delete) para preservar la integridad referencial de los datos.
