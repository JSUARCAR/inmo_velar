...
### Objetivo
Gestionar y supervisar de forma integral la información y operaciones relacionadas con **Proveedores** dentro del ecosistema de Inmobiliaria Velar.

### Alcance
Este módulo abarca la creación, consulta, modificación y seguimiento de todos los registros de Proveedores, permitiendo un control centralizado.
...
> En el contexto de **Proveedores**, un registro representa una entidad única que atraviesa un ciclo de vida definido por reglas de negocio estrictas.
...
- **Ruta de acceso**: `Menú Principal → Proveedores`
...
La pantalla principal de **Proveedores** se compone de los siguientes elementos:
...
### 5.1. Crear un registro de Proveedores
...
```mermaid
flowchart TD
    A[Inicio] --> B(Ingresar a Proveedores)
...
> Todo cambio realizado en el módulo de **Proveedores** genera automáticamente un registro en el log de auditoría. Los registros no se eliminan físicamente (Soft-Delete) para preservar la integridad referencial de los datos.
