...
### Objetivo
Gestionar y supervisar de forma integral la información y operaciones relacionadas con **IPC / Incrementos** dentro del ecosistema de Inmobiliaria Velar.

### Alcance
Este módulo abarca la creación, consulta, modificación y seguimiento de todos los registros de IPC / Incrementos, permitiendo un control centralizado.
...
> En el contexto de **IPC / Incrementos**, un registro representa una entidad única que atraviesa un ciclo de vida definido por reglas de negocio estrictas.
...
- **Ruta de acceso**: `Menú Principal → IPC / Incrementos`
...
La pantalla principal de **IPC / Incrementos** se compone de los siguientes elementos:
...
### 5.1. Crear un registro de IPC / Incrementos
...
```mermaid
flowchart TD
    A[Inicio] --> B(Ingresar a IPC / Incrementos)
...
> Todo cambio realizado en el módulo de **IPC / Incrementos** genera automáticamente un registro en el log de auditoría. Los registros no se eliminan físicamente (Soft-Delete) para preservar la integridad referencial de los datos.
