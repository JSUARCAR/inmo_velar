...
### Objetivo
Gestionar y supervisar de forma integral la información y operaciones relacionadas con **Incidentes** dentro del ecosistema de Inmobiliaria Velar.

### Alcance
Este módulo abarca la creación, consulta, modificación y seguimiento de todos los registros de Incidentes, permitiendo un control centralizado.
...
> En el contexto de **Incidentes**, un registro representa una entidad única que atraviesa un ciclo de vida definido por reglas de negocio estrictas.
...
- **Ruta de acceso**: `Menú Principal → Incidentes`
...
La pantalla principal de **Incidentes** se compone de los siguientes elementos:
...
### 5.1. Crear un registro de Incidentes
...
```mermaid
flowchart TD
    A[Inicio] --> B(Ingresar a Incidentes)
...
> Todo cambio realizado en el módulo de **Incidentes** genera automáticamente un registro en el log de auditoría. Los registros no se eliminan físicamente (Soft-Delete) para preservar la integridad referencial de los datos.
