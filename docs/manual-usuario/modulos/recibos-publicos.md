...
### Objetivo
Gestionar y supervisar de forma integral la información y operaciones relacionadas con **Recibos Públicos** dentro del ecosistema de Inmobiliaria Velar.

### Alcance
Este módulo abarca la creación, consulta, modificación y seguimiento de todos los registros de Recibos Públicos, permitiendo un control centralizado.
...
> En el contexto de **Recibos Públicos**, un registro representa una entidad única que atraviesa un ciclo de vida definido por reglas de negocio estrictas.
...
- **Ruta de acceso**: `Menú Principal → Recibos Públicos`
...
La pantalla principal de **Recibos Públicos** se compone de los siguientes elementos:
...
### 5.1. Crear un registro de Recibos Públicos
...
```mermaid
flowchart TD
    A[Inicio] --> B(Ingresar a Recibos Públicos)
...
> Todo cambio realizado en el módulo de **Recibos Públicos** genera automáticamente un registro en el log de auditoría. Los registros no se eliminan físicamente (Soft-Delete) para preservar la integridad referencial de los datos.
